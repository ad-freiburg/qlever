...
`$QLEVER_HOME/qlever-code`, and the docker image is called `qlever`.
        ...
        chmod o+w . && docker run -it --rm -v $QLEVER_HOME/qlever-indices/scientists:/index --entrypoint bash qlever -c "cd /index && cat scientists.nt | IndexBuilderMain -F ttl -f - -l -i scienists -w scientists.wordsfile.tsv -d scientists.docsfile.tsv | tee scienists.index-log.txt"
