...
FileBuffer::FileBuffer(Allocator& allocator, FileBufferType type,
                       uint64_t user_size)
...
void FileBuffer::ReallocBuffer(idx_t new_size) {
  ...
  if (internal_buffer) {
    new_buffer =
        allocator.ReallocateData(internal_buffer, internal_size, new_size);
  } else {
...
FileBuffer::MemoryRequirement FileBuffer::CalculateMemory(uint64_t user_size) {
  ...
  if (type == FileBufferType::TINY_BUFFER) {
    // We never do IO on tiny buffers, so there's no need to add a header or
    // sector-align.
    ...
    result.alloc_size =
        AlignValue<idx_t, Storage::SECTOR_SIZE>(result.header_size + user_size);
  }
  return result;
}
...
void FileBuffer::Clear() { memset(internal_buffer, 0, internal_size); }
...
void FileBuffer::Initialize(DebugInitialize initialize) {
  ...
  uint8_t value =
      initialize == DebugInitialize::DEBUG_ZERO_INITIALIZE ? 0 : 0xFF;
