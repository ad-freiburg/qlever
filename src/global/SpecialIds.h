...
static const inline ad_utility::HashMap<std::string, IdNoLocalVocab> specialIds = []() {
  ad_utility::HashMap<std::string, IdNoLocalVocab> result{
  ...
  auto undefTypeButNotUndefValue = [](IdNoLocalVocab id) {
    return !Id::equalByBits(id, Id::makeUndefined()) && id.getDatatype() == Datatype::Undefined;
  };
  AD_CORRECTNESS_CHECK(std::ranges::all_of(values, undefTypeButNotUndefValue));
  ad_utility::HashSet<IdNoLocalVocab> uniqueIds(values.begin(), values.end());
