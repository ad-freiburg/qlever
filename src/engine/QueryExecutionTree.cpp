...
nlohmann::json QueryExecutionTree::writeJsonTable(
    ...
    const vector<std::optional<pair<size_t, ResultTable::ResultType>>>& validIndices) const {
  shared_ptr<const ResultTable> res = getResult();
...
void QueryExecutionTree::writeTable(
    ...
    const vector<std::optional<pair<size_t, ResultTable::ResultType>>>& validIndices,
    ...
    for (size_t j = 0; j < validIndices.size(); ++j) {
      if (!validIndices[j]) {
        ...
        switch (val.second) {
          ...
          case ResultTable::ResultType::LOCAL_VOCAB: {
            out << res->idToOptionalString(data(i, val.first))
                       .value_or("");
            break;
          }
          default:
