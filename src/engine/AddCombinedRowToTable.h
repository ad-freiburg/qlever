 ...
 public:
  ...
  void flush() {
    ...
    auto writeJoinColumn = [&result, &isUndef, oldSize, this](
      ...
      auto leftPayloads = colLeft.rawPayloads();
      auto leftTypes = colLeft.rawDatatypes();
      auto rightPayloads = colRight.rawPayloads();
      auto rightTypes = colRight.rawDatatypes();
      // TODO<joka921> Implement prefetching.
      decltype(auto) resultCol = result.getColumn(resultColIdx);
      auto resultPayloads = resultCol.rawPayloads();
      auto resultTypes = resultCol.rawDatatypes();
      size_t numUndef = 0;
    ...
    auto writeNonJoinColumn = ad_utility::ApplyAsValueIdentity{
        [&result, &isUndef, oldSize, this](auto isColFromLeft, size_t colIdx,
          ...
          auto payloads = col.rawPayloads();
          auto types = col.rawDatatypes();
          // TODO<joka921> Implement prefetching.
          decltype(auto) resultCol = result.getColumn(resultColIdx);
          auto resultPayloads = resultCol.rawPayloads();
          auto resultTypes = resultCol.rawDatatypes();
