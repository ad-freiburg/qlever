...
class Date {
 private:
  ...
  // Used as a tag for certain unchecked constructor that may not be called from
  // outside.
 ...
 private:
  // Construct without checking the range, and without performing the shift for
  // the year. This constructor is just an implementation detail.
  constexpr Date(short year, signed char month, signed char day,
                 NoShiftOrCheckTag)
      : _year{year}, _month{month}, _day{day} {}
