...
#ifndef QLEVER_CANCELABLEPARALLELSORT_H
#define QLEVER_CANCELABLEPARALLELSORT_H
#include <vector>

// clang-format off
// the include order is crucial since we are messing with internal headers
...
namespace cancelableParallelSort {

using __gnu_parallel::_ThreadIndex;
using __gnu_parallel::sequential_tag;
using __gnu_parallel::_Settings;

struct ParallelSorter {
  ad_utility::TimeoutChecker* _checker;
  ad_utility::sequential::TimeoutedAlgorithms _alg;
  ParallelSorter(ad_utility::TimeoutChecker* c) :_checker(c), _alg{c} {}
...
struct _PMWMSSortingData
{
  ...
  __gnu_parallel::_ThreadIndex _M_num_threads;
                    ...
                    _DifferenceTp __num_samples)
{
  ...
  __gnu_parallel::_ThreadIndex __iam = omp_get_thread_num();
...
#       pragma omp barrier
    ...
    if (__iam < __sd->_M_num_threads - 1)
      __gnu_parallel::multiseq_partition(__seqs.begin(), __seqs.end(),
...
struct __possibly_stable_sort<true, _RAIter, _Compare>
{
  void operator()(const _RAIter& __begin,
                  const _RAIter& __end, _Compare& __comp, ad_utility::sequential::TimeoutedAlgorithms* = nullptr) const
...
{
  void operator()(const _RAIter __begin,
                  const _RAIter __end, _Compare& __comp, ad_utility::sequential::TimeoutedAlgorithms* alg) const
  { alg->sort(__begin, __end, __comp); }
};
...
parallel_sort_mwms_pu(_PMWMSSortingData<_RAIter>* __sd,
                      _Compare& __comp, bool* cancelPtr)
{
  bool& cancelRef = *cancelPtr;
  typedef std::iterator_traits<_RAIter> _TraitsType;
  typedef typename _TraitsType::value_type _ValueType;
  typedef typename _TraitsType::difference_type _DifferenceType;
...
#       pragma omp single
    ...
    parallel_sort_mwms_pu<__stable, __exact>(&__sd, __comp, cancelPtr);
  } //parallel
  ...
  if (canceled) {
    throw std::runtime_error{"Exception was raised during parallel sort (probably timeout exception)"};
  }
}
...
void sort(_RAIter __begin, _RAIter __end, _Compare __comp,
          _ThreadIndex __num_threads) {
  parallel_sort_mwms<false, true>(__begin, __end, __comp, __num_threads);
}
...
#endif  // QLEVER_CANCELABLEPARALLELSORT_H
