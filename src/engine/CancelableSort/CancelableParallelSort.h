...
struct ParallelSorter {
  ...
  static void __determine_samples(_PMWMSSortingData<_RAIter>* __sd,
...
#pragma omp barrier
  ...
  struct __possibly_stable_sort<true, _RAIter, _Compare> {
    void operator()(
        const _RAIter& __begin, const _RAIter& __end, _Compare& __comp,
        ad_utility::sequential::TimeoutedAlgorithms* = nullptr) const {
  ...
  struct __possibly_stable_sort<false, _RAIter, _Compare> {
    void operator()(const _RAIter __begin, const _RAIter __end,
                    _Compare& __comp,
                    ad_utility::sequential::TimeoutedAlgorithms* alg) const {
...
#pragma omp single
    ...
    if (canceled) {
      throw std::runtime_error{
          "Exception was raised during parallel sort (probably timeout "
          "exception)"};
    }
  }
