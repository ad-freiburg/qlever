...
// from the `ValueId` alone (grid cell of a WKT literal, coordinates of a
// `GeoPoint`, see `GeoCellIdPrefilter`). It complements the block prefilter of
// an `IndexScan`: the block prefilter can only drop whole blocks, and for
...
class GeoRectangleRowFilter : public Operation {
 private:
  ...
  ad_utility::GeoCellIdPrefilter prefilter_;
