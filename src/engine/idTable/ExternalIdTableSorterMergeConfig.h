...
//
// NOTE: The sorter itself is wired up to this configuration in a follow-up PR,
// so nothing else in `src/` calls the functions below yet.
...
// the merges cannot starve each other. It is however *not* safe for a
// *blocking* consumer of a merge to run on one of the threads of that merge's
// own executor, because it would then occupy a thread that the merge needs in
...
// data (which takes 3.2 seconds), measured with a dedicated benchmark of the
...
// does not exist in the C++17 backports mode, where the merge runs serially and
