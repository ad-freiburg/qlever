...
class IdTable {
 ...
 public:
  ...
  CPP_template(typename = void) (requires columnsAreAllocatable CPP_and
                  std::is_default_constructible_v<Allocator>)
  explicit IdTable(size_t numColumns) : IdTable(numColumns, Allocator{}) {}

  CPP_template(typename = void) (requires(columnsAreAllocatable))
  ...
  CPP_template(
      typename ColT) (requires ql::ranges::forward_range<ColT> CPP_and CPP_NOT(isView))
  ...
  CPP_template(
      typename =
          void) (requires CPP_NOT(isView) CPP_and columnsAreAllocatable CPP_and
                  std::is_default_constructible_v<Allocator>)
  IdTable() : IdTable{NumColumns, Allocator{}} {};

  CPP_template(
      typename = void) (requires CPP_NOT(isView) CPP_and columnsAreAllocatable)
 ...
 public:
  ...
  CPP_template(typename =
                   void) (requires columnsAreAllocatable CPP_and isDynamic)
  void addEmptyColumn() {
  ...
  CPP_template(typename = void) (requires(!isView))
  row_reference_restricted operator[](size_t index) {
    return *(begin() + index);
  }
  ...
  CPP_template(typename = void) (requires(!isView))
  row_reference_restricted at(size_t row) {
  ...
  // ("deducing this").
  CPP_template(typename = void) (requires(!isView))
  row_reference_restricted front() {
    return (*this)[0];
  }
  ...
  CPP_template(
      typename RowLike) (requires CPP_NOT(isView)
                  CPP_and ql::ranges::random_access_range<RowLike>
                      CPP_and ql::concepts::same_as<
                          ql::ranges::range_value_t<RowLike>, T>)
  void push_back(const RowLike& newRow) {
  ...
  CPP_template(typename = void) (requires(!isView))
  void push_back(const std::initializer_list<T>& newRow) {
  ...
  CPP_member auto moveOrClone() const& -> CPP_ret(IdTable)(requires isCloneable)
  {
    return clone();
  }
  ...
  CPP_template(
      typename = void) (requires(!std::is_copy_constructible_v<ColumnStorage>))
  ...
  //       write.
  CPP_template(
      int NewNumColumns) (requires(isDynamic || NewNumColumns == NumColumns ||
                       NewNumColumns == 0) CPP_and CPP_NOT(isView))
  ...
  // from.
  CPP_template(typename = void) (requires(!isView))
  IdTable<T, 0, ColumnStorage> toDynamic() && {
  ...
  CPP_template(typename = void) (requires isDynamic)
  void deleteColumn(ColumnIndex colIdx) {
  ...
  CPP_member auto erase(const iterator& beginIt, const iterator& endIt) -> CPP_ret(
      void)(requires(!isView))
  {
  ...
  CPP_member auto erase(const iterator& it) -> CPP_ret(void)(requires(!isView))
  {
    erase(it, it + 1);
  }
  ...
  // testing.
  CPP_template(typename = void) (requires(!isView))
  bool operator==(const IdTable& other) const {
