...
Awaitable<void> Server::process(
  ...
  //
  // 2. If value is `std::nullopt`, check if the given key-value pair exists,
  // and if yes, return the value (not really needed, but just so that the
  // interface is the same as in the first case), and `std::nullopt` otherwise.
  //
