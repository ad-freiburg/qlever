  ...
  for (size_t round = 0; unlimitedRounds || round < maxRounds; ++round) {
    ...
    for (auto& pr : prepared) {
      ...
      if (newFromRule > 0) {
        ...
        // in the same round see correct
        // `containsDuplicatesWithDifferentGraphs_` flags when reading index
        // blocks.  Without this, the compressed-block reader fires an
        // AD_EXPENSIVE_CHECK when it merges the freshly-inserted delta triples
        // with a block that was not yet marked as containing graph-level
        // duplicates.
        deltaTriples.updateAugmentedMetadata();
