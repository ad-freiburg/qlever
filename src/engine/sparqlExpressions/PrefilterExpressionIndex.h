...
// prefiltered via the Z-order ranges of the rectangle (their IDs are Z-order
// codes of the coordinates). WKT literals of a vocabulary with a geo cell grid
// are prefiltered via the cell bits of their IDs (see `GeoCellGrid`); without
// a grid, their coordinates cannot be seen from their IDs, so all blocks of
// the `VocabIndex` datatype are kept. Values of all other datatypes cannot be
// geometries that satisfy a spatial condition and are pruned (blocks with
// mixed datatypes are always kept by the surrounding framework).
