...
inline auto valueGetterGenerator =
    []<typename ValueGetter, SingleExpressionResult Input>(
        ...
        std::remove_reference_t<SparqlExpression::CancellationHandle>* handle, Input&& input,
        ValueGetter&& valueGetter) {
      auto transformation = [ context, valueGetter, handle ]<typename I>(I && i)
          requires std::invocable<ValueGetter, I&&, EvaluationContext*> {
        handle->throwIfCancelled("Value getter generator");
...
auto applyOperation(size_t numElements, Operation&&, EvaluationContext* context,
                    std::remove_reference_t<SparqlExpression::CancellationHandle>* handle,
                    Operands&&... operands) {
  ...
  auto getValue = std::bind_front(valueGetterGenerator, numElements, context, handle);
