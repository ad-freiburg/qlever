...
inline auto valueGetterGenerator =
    []<typename ValueGetter, SingleExpressionResult Input>(
      ...
      auto transformation =
          [context, valueGetter, handle]<typename I>(I&& i)
...
auto applyOperation(
    size_t numElements, Operation&&, EvaluationContext* context,
    std::remove_reference_t<SparqlExpression::CancellationHandle>* handle,
    Operands&&... operands) {
  ...
  auto getValue =
      std::bind_front(valueGetterGenerator, numElements, context, handle);
