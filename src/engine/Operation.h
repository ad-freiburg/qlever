...
class Operation {
 ...
 public:
  ...
  virtual std::vector<QueryExecutionTree*, qlever::Allocator<QueryExecutionTree*>> getChildren() = 0;

  /// get non-owning constant pointers to all the held subtrees to actually use
  /// the Execution Trees as trees
  viauto interm = const_cast<Operation*>(this)->getChildren();
    return std::vector<const QueryExecutionTree*, qlever::Allocator<const QueryExecutionTree*>>(
        interm.begin(), interm.end(), allocator().as<const QueryExecutionTree*>
    return std::vector<const QueryExecutionTree*, qlever::Allocator<const QueryExecutionTree*>>(interm.begin(), interm.end());
  }
