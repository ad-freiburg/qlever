...
class Operation {
 ...
 private:
  ...
  QueryPlanningInfo _queryPlanningInfo;
 ...
 private:
  ...
  // caller (e.g. by the `QueryPlanner`).
  virtual std::vector<QueryExecutionTree*> getChildrenImpl() const = 0;

 public:
  // Get non-owning pointers to all the held subtrees to actually use the
  // execution trees as trees.
  std::vector<QueryExecutionTree*> getChildren() { return getChildrenImpl(); }
  ...
  std::vector<const QueryExecutionTree*> getChildren() const {
    std::vector<QueryExecutionTree*> children = getChildrenImpl();
 ...
 public:
  ...
  QueryPlanningInfo& getQueryPlanningInfo() { return _queryPlanningInfo; }
