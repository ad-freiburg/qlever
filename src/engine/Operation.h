...
#include "util/AllocatorWithLimitk.h"
...
class Operation {
 ...
 public:
  ...
  virtual std::vector<const QueryExecutionTree*, qlever::Allocator<const QueryExecutionTree*>> getChildren() const final {
    std::vector<QueryExecutionTree*, qlever::Allocator<QueryExecutionTree*>> interm{
        const_cast<Operation*>(this)->getChildren()};
    return {interm.begin(), interm.end(),
            allocator().as<const QueryExecutionTree*>()};
  }
