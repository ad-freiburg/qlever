...
// thread instead of blocking it. The output blocks are pushed to the `Sink`,
// which is where the back-pressure and the order of the blocks live, see
// `SinkConcept`.
//
...
// on `executor_` throughout and only hops to the sink for the short bookkeeping
// of an `asyncPush`, see the IMPORTANT note at `SinkConcept`.
//
...
CPP_template(bool moveElements, typename Input, typename Comparator,
 ...
 private:
  ...
  // then posts (and never invokes inline) the handler there, see the IMPORTANT
  // note at `SinkConcept`.
  class ChunkTask : public std::enable_shared_from_this<ChunkTask> {
   ...
   public:
    ...
    // throws; all exceptions are forwarded to the sink via
    // `asyncPushException`.
    //
   ...
   private:
    ...
    void fail(std::exception_ptr exception) noexcept {
      ...
      try {
        ...
        // so the only way it can fail is that memory is exhausted. Ignore that,
        // because the chunk has to be finished in any case, see below.
      }
      if (!wasForwarded) {
        finish();
      }
    ...
    // does. NOTE: This has to happen on every path, because a sink that waits
    // for this chunk would wait forever otherwise.
  ...
  // index: a chunk holds its permit while its producer is suspended on a sink
  // that has no room, so if a higher chunk could start before a lower one, all
  // permits could be held by producers of chunks that the consumer does not
  // read yet, and the merge would deadlock.
  //
  ...
  //
  // NOTE: The forwarding typically allocates a handler of its own, so the only
  // way it can fail is that memory is exhausted, in which case the consumer
  // sees the end of the output instead of the exception.
  void forwardExceptionToSink(std::exception_ptr exception) noexcept {
    ad_utility::ignoreExceptionIfThrows(
        [this, &exception] {
        ...
        "Forwarding an exception of the chunk dispatcher to the sink of "
