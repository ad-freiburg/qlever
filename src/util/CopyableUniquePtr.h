...
CPP_template(
    typename T,
    typename Deleter =
        std::default_delete<T>) (requires std::is_copy_constructible_v<T>)
class CopyableUniquePtr : public std::unique_ptr<T, Deleter> {
 ...
 public:
  ...
  // Json serialization.
  CPP_template_2(typename S)(requires OrderedOrUnorderedJson<S>)
  friend void to_json(S& j, const CopyableUniquePtr& p) {
    /*
