...
#ifndef QLEVER_SRC_UTIL_FLATTENITERATOR_H
#define QLEVER_SRC_UTIL_FLATTENITERATOR_H
...
// A view over a nested container (e.g. a vector of vectors) that iterates all
// elements of the inner containers in sequence, see `detail::FlattenIterator`.
template <typename OuterIt, typename InnerIt>
class FlattenView
    : public ql::ranges::view_interface<FlattenView<OuterIt, InnerIt>> {
  using Iterator = detail::FlattenIterator<OuterIt, InnerIt>;
  Iterator begin_{};
  Iterator end_{};

 public:
  FlattenView() = default;
  FlattenView(OuterIt outerBegin, OuterIt outerEnd)
      : begin_{outerBegin, outerEnd}, end_{outerEnd, outerEnd} {}
...
#endif  // QLEVER_SRC_UTIL_FLATTENITERATOR_H
