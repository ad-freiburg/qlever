...
// reused over and over again, for example if there are many `std::vector`s over
// time for which we know that the capacity will be the same, but only few of
...
// to a guaranteed upper bound of their actual size. A simpler approach would be
// to reuse the hash maps directly, but once this is not feasible, this
// allocator can come in handy.
