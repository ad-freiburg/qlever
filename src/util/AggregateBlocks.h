...
template <typename T, typename ElementAction, typename BlockAction, typename Projection, typename Equality = std::equal_to<>>
CoroToStateMachine<T> aggregateBlocks(ElementAction elementAction, BlockAction blockAction, Projection projection, Equality equality = Equality{} ) {
  if (!co_await ad_utility::valueWasPushedTag) {
    co_return;
  }
  auto currentElement = std::invoke(projection, co_await ad_utility::nextValueTag);
  do {
