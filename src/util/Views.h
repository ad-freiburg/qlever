  ...
  class Iterator {
   public:
    ...
    decltype(auto) operator*() const {
      ...
      AD_LOG_INFO << "dereference\n";
      if (!std::exchange(doneWasChecked_, true)) {
        doneAfterIncrement_ = (*pred_)(res);
      }
      return AD_FWD(res);
    }
...
// TODO<joka921> Move this into its own header.
// Helper function
namespace views {
struct take_until_inclusive_fn {
  template <ql::ranges::viewable_range R, typename Pred>
  ...
  constexpr auto operator()(Pred pred) const {
    return ::ranges::make_view_closure(
        [pred = std::move(pred)]<ql::ranges::viewable_range R>(R&& r) {
          return TakeUntilInclusiveView(AD_FWD(r), pred);
        });
  }
};

inline constexpr take_until_inclusive_fn takeUntilInclusive;
}  // namespace views
}  // namespace ad_utility
