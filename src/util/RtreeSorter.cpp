...
OrderedBoxes SortInput(const std::string& onDiskBase, const std::string& fileSuffix, size_t M,
    ...
    auto maxRamForSorter =
            std::ceil(((double) maxBuildingRamUsage < 9999999999.0 ? (double) maxBuildingRamUsage
                                                          : 9999999999.0) /
                      3.0);
    ...
    if (workInRam) {
        for (RTreeValue element : rectsD0Basic) {
            ...
            if (xSize == 0) {
              ...
              boundingBox = BasicGeometry::CombineBoundingBoxes(boundingBox, element.box);
            }
            xSize++;
        }
...
/*OrderedBoxes SortInput(const std::filesystem::path& onDiskBase, size_t M, uintmax_t
                                                                              maxBuildingRamUsage, bool workInRam) { if (workInRam) {
        return InternalSort(onDiskBase, M); } else { return ExternalSort(onDiskBase, M,
                        maxBuildingRamUsage);
  }
}*/
