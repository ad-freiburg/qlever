...
struct Pipeline {
  ...
  Pipeline(bool ordered, std::vector<size_t> t_parallelisms, F1 f1, F2 f2, Fs... fs): m_functions{std::move(f1), std::move(f2), std::move(fs)...}, _ordered(ordered) {
    ...
    for (auto p : t_parallelisms) {
      ...
      _queues.push_back(std::make_unique<ad_utility::TaskQueue<true>>(2 * p + 1, p));
    }
  ...
  auto popManually() {
    return _queues.back()->popManually();
  }
  ...
  std::function<void(void)> makeChainedFunction(size_t requestId, Ts&&... input) {
    if constexpr (I == 0) {
      return std::function([&]() {
        size_t requestCounter = 0;
        while (auto optional = get<0>()()) {
          _queues[1]->push(makeChainedFunction<1>(requestCounter++, std::move(*optional)));
        }
      });
      ...
      return std::function(
          [this, requestId, ...input = std::forward<Ts>(input)]() mutable {
             ...
             if (_ordered) {
               ...
               _pushedNextOrderedRequest[I].wait(l, [requestId, this](){return requestId == _nextRequestId[I];});
               _nextRequestId[I]++;
               _queues[I + 1]->push(
