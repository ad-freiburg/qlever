...
// next step and resolves to what that step yielded, `cancel()` aborts the
// request and thereby closes the connection. A step yields one of
...
EM_JS(void, qleverFetch, (EM_VAL handle), {
    ...
    try {
      ...
      if (fetched.body) {
        ...
        while (true) {
          ...
          // `read()` resolves to `{done: false, value}` with the next piece of
          // the body (a `Uint8Array`), and to `{done: true}` once the body has
  ...
  response.next = async () => {
    // `steps` is an async generator: each `steps.next()` runs its body up to
    // the next `yield` and resolves to `{done: false, value}` with what it
...
//
// NOTE: The thread is created lazily, by the first request. When that request
// comes from a thread other than the main thread, Emscripten defers the
// creation to the main thread's event loop, so the network thread only comes up
// once that loop runs. A build whose main thread may block (one without
// `-sPROXY_TO_PTHREAD`) must therefore not keep it blocked while the first
...
// Cancel the request of a response and delete it. Both may only happen on the
// network thread, so both are proxied there.
struct CancelAndDeleteResponse {
...
using ResponsePtr = std::unique_ptr<val, CancelAndDeleteResponse>;
...
// Destroying it cancels the request, so that a response that nobody reads does
...
// iterated destroys its parameters but never runs its body. That way, a
// response that is not read at all still cancels its request (destroying
// `request` aborts the `fetch` and thereby closes the connection).
...
// Complain if the request is issued from the main thread of a browser, which
// may not block. A response can only arrive once that thread returns to its
// event loop, so waiting for it there freezes the page. Without a preloaded Web
// Worker (see `-sPTHREAD_POOL_SIZE`), the very first request never completes
// at all: it creates the network thread, whose Web Worker only comes up once
// the main thread reaches its event loop.
//
// The main thread of Node.js may block, and so may a Web Worker (which is what
// Emscripten's threads are), hence the check for a browser rather than just
// for the main thread. This is the same distinction that Emscripten itself
// makes in `emscripten_check_blocking_allowed`, which only warns.
//
