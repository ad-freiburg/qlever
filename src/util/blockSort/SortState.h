...
class SortState {
 public:
  ...
  RangeIt tailRange_;
  Compare cmp_;
  // The seed that the scratch buffers are initialized from, see
  // `ScratchBuffers`. It is a copy of the first element and not that element
  // itself, because `*first` is not an object at all if the `Iterator` hands
  // out proxy references, see `util/blockSort/SortPrimitives.h`. It has to be
  // declared before `buffers_`, which is initialized from it.
  Value bufferSeed_;
