...
void divideSort(State& state, Iterator first, Iterator last, uint32_t level,
  ...
  //
  // NOTE: The pivot is a copy and not a reference to `*first`, because `*first`
  // is not an object at all if the `Iterator` hands out proxy references, see
  // `util/blockSort/SortPrimitives.h`. The copy is made once per partition and
  // is therefore not worth a special case for the other iterators.
