...
//
// NOTE: The scheme is stored by name in the meta data of an index (see
// `GeoCellGridScheme` below), so the names must never change.
...
class GeoCellGrid {
 ...
 public:
  ...
  // apart again. The cell index must be at most `maxCellIndex()` (for the
  // shifted quadtrees, the cells of the second and third copy are larger than
  // `sentinelCell()`) and the position must be smaller than `maxNumWords()`.
