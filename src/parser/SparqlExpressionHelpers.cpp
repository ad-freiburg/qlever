...
void getIdsFromVariableImpl(VectorWithMemoryLimit<StrongId>& result,
  ...
  if (!context->_variableToColumnAndResultTypeMap.contains(variable._variable)) {
    throw std::runtime_error(
        "Variable " + variable._variable +
        " could not be mapped to context column of expression evaluation");
  }

  size_t columnIndex = context->_variableToColumnAndResultTypeMap.at(variable._variable).first;
