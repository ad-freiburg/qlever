...
struct ProxyConfiguration {
  ...
  std::vector<std::pair<std::string, Variable>> payloadVariables_;
  ...
  std::vector<std::pair<std::string, Variable>> resultVariables_;

  // Parameters of the proxy request, which are sent as URL query parameters.
  std::vector<std::pair<std::string, std::string>> parameters_;
};

// Class for a parsed magic `SERVICE qlproxy:` request. Sends payload bindings
...
struct ProxyQuery : MagicServiceQuery {
  ...
  // Payload variables to send to the remote endpoint.
  // Extracted from predicates like qlproxy:payload_<name>.
  std::vector<std::pair<std::string, Variable>> payloadVariables_;

  // Result variables to receive from the remote endpoint.
  // Extracted from predicates like qlproxy:result_<name>.
  std::vector<std::pair<std::string, Variable>> resultVariables_;

  // Static parameters sent as URL query parameters.
  // Extracted from predicates like qlproxy:param_<name>.
  std::vector<std::pair<std::string, std::string>> parameters_;
