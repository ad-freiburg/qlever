...
ExpressionPtr Visitor::processIriFunctionCall(
  ...
  // Tensor functions.
  static const UnaryFuncTable tensorFuncs{
      {"tensor", &makeTensorFromStringExpression},
  };
...
GraphPatternOperation Visitor::visit(Parser::ServiceGraphPatternContext* ctx) {
  ...
  } else if (serviceIri.toStringRepresentation() == TENSOR_INDEX_IRI) {
    return visitMagicServiceQuery<parsedQuery::TensorIndexQuery>(ctx);
