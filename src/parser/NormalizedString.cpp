...
NormalizedString fromStringUnsafe(std::string_view input) {
  NormalizedString normalizedString; // Create new empty NormalizedString
  normalizedString.resize(input.size());

  std::transform(input.begin(), input.end(), normalizedString.begin(), [](char c){return NormalizedChar{c};});
...
std::string_view asStringView(NormalizedStringView normalizedStringView) {
  return {reinterpret_cast<const char*>(normalizedStringView.data()), normalizedStringView.size()};
}
