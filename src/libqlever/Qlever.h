...
struct IndexRebuildConfig {
  ...
  // The base name of the new index during the rebuild (in the temporary
  // directory).
...
class Qlever {
 ...
 public:
  ...
  // old index (so that a later restart loads the latest index).
  //
  // The renames keep the open file handles of both indexes valid, so running
  ...
  // state still refers to a consistent old index, but the on-disk layout may
  // have to be repaired manually before the next restart.
