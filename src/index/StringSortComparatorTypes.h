...
/**
 * @brief Compares strings according to the collation of the held
 * `LocaleManagerT`, e.g. strings from the text index vocabulary. To compare
 * components of RDF triples use the `TripleComponentComparatorImpl` defined
 * below.
 *
 * @tparam LocaleManagerT The locale manager whose collation is used. This is
 * `LocaleManagerICU` in a regular build and `LocaleManagerNoICU` in an ICU-free
 * build. Templating on it (instead of using the `#ifdef`) allows both variants
 * to be instantiated and tested regardless of the build configuration.
 */
 ...
 *
 * @tparam LocaleManagerT see `SimpleStringComparatorImpl` above.
 */
template <typename LocaleManagerT>
