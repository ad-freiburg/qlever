...
// _____________________________________________________________________________
cppcoro::generator<std::string> IndexImpl::allPrefixes() {
  static_assert(
      MIN_WORD_PREFIX_SIZE >= 1 && MIN_WORD_PREFIX_SIZE <= 4,
      "If you need this to be changed, please contact the developers");
  char firstChar = 'a';
  char lastChar = 'z';
  std::string prefix(MIN_WORD_PREFIX_SIZE, firstChar);
  while (true) {
    co_yield prefix;
    // Increment "counter" by one.
    for (size_t i = MIN_WORD_PREFIX_SIZE; i > 0; --i) {
      if (prefix[i - 1] < lastChar) {
        prefix[i - 1]++;
        break;
      }
      prefix[i - 1] = firstChar;
    }
...
// Check if the prefixes generated by `allPrefixes` are sorted according to the
// given comparator.
bool IndexImpl::areAllPrefixesSorted(auto comparator) {
  std::string first;
  for (auto second : allPrefixes()) {
...
void IndexImpl::calculateBlockBoundariesImpl(
  ...
  if (!areAllPrefixesSorted(index.textVocab_.getCaseComparator())) {
  ...
  // Iterate over all prefixes generated by `allPrefixes`.
  auto forcedBlockStarts = allPrefixes();
  auto forcedBlockStartsIt = forcedBlockStarts.begin();

  // If there is a prefix `forcedBlockStarts`, for which the `SortKey` is a
  // prefix of `prefixSortKey`, then set `prefixSortKey` to that `SortKey` and
  // `prefixLength` to `MIN_WORD_PREFIX_SIZE`. This ensures that the blocks
  // corresponding to these prefixes are never split up because of Unicode
  // ligatures.
  ...
  auto getLengthAndPrefixSortKey = [&](WordVocabIndex i) {
    ...
    // If we are in a block where one of the prefixes generated by
    // `allPrefixes`, use those as the block start.
