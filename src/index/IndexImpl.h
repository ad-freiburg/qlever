...
class IndexImpl {
 ...
 private:
  ...
  inline static const ad_utility::HashMap<std::string,
      ...
      turtleParserIntegerOverflowBehaviorMap_{
          {"overflowing-integers-throw",
           TurtleParserIntegerOverflowBehavior::Error},
          {"overflowing-integers-become-doubles",
           TurtleParserIntegerOverflowBehavior::OverflowingToDouble},
          {"all-integers-become-doubles",
           TurtleParserIntegerOverflowBehavior::AllToDouble}};

  /*
  Some of the variables, that will be set by the `ConfigManager` generated by
  `generateConfigManagerForIndexBuilderSettings`, after its parse function was
  called. The remaining variables, it writes to, are member variables of
  `IndexImpl`.
  */
  struct IndexBuilderSettingsVariables {
    std::vector<std::string> prefixesExternal_;
    std::vector<std::string> languagesInternal_;
    std::string localLang_;
    std::string localCountry_;
    bool localIgnorePunctuation_;
    std::string parserIntegerOverflowBehavior_;
  };

  /*
  @brief Generate the `ConfigManager`, and some of the variables it writes to,
  that describes the index builder settings. The remaining variables, it writes
  to, are member variables of `IndexImpl`.
  */
  std::pair<ad_utility::ConfigManager,
            std::unique_ptr<IndexBuilderSettingsVariables>>
  generateConfigManagerForIndexBuilderSettings();
