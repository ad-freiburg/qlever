...
CompressedRelationReader::asyncParallelBlockGenerator(
  ...
  auto readAndDecompressBlock = [&]()
    ...
    auto decompressedBlock =
        decompressBlock(compressedBlock, blockMetadata.numRows_, {locatedTriples, blockMetadata.blockIndex_);
  ...
  auto setTimer = ad_utility::makeOnDestructionDontThrowDuringStackUnwinding(
      [&details, &popTimer]() {
      details.blockingTime_ = popTimer.msecs(); });
  ...
  CompressedRelationReader::FilterDuplicatesAndGraphs::
      blockNeedsFilteringByGraph(const CompressedBlockMetadata& metadata)
          const {
  ...
  // _____________________________________________________________________________
  bool
  CompressedRelationReader::FilterDuplicatesAndGraphs::filterByGraphIfNecessary(
  ...
  bool CompressedRelationReader::FilterDuplicatesAndGraphs::postprocessBlock(
    ...
    bool filteredByDuplicates =
        filterDuplicatesIfNecessary(block, blockMetadata);
  ...
  CompressedRelationReader::IdTableGenerator CompressedRelationReader::lazyScan(
    ...
    AD_CORRECTNESS_CHECK(numBlocksTotal ==
                             (details.numBlocksRead_ +
                              details.numBlocksSkippedBecauseOfGraph_) ||
  ...
  CompressedRelationReader::getBlocksForJoin(
    ...
    return {findMatchingBlocks(blocksWithFirstAndLastId1,
                               blocksWithFirstAndLastId2),
            findMatchingBlocks(blocksWithFirstAndLastId2,
                               blocksWithFirstAndLastId1)};
  }
  ...
  IdTable CompressedRelationReader::scan(
    ...
    if (limitOffset._limit.has_value()) {
      upperBoundSize = std::min(
          upperBoundSize, static_cast<size_t>(limitOffset._limit.value()));
    }
  ...
  DecompressedBlock CompressedRelationReader::readPossiblyIncompleteBlock(
    ...
    std::ranges::copy(ad_utility::integerRange(
    ...
    auto filterColumn = [&block, &beginIdx, &endIdx](
                            std::optional<Id> relevantId, size_t columnIdx) {
      if (!relevantId.has_value()) {
        return;
      }
      const auto& column = block.getColumn(columnIdx);
      auto matchingRange =
          std::ranges::equal_range(column.begin() + beginIdx,
                                   column.begin() + endIdx, relevantId.value());
      beginIdx = matchingRange.begin() - column.begin();
      endIdx = matchingRange.end() - column.begin();
    };

    // The order of the calls is important because the block is sorted by
    // [col0Id, col1Id, col2Id].
  ...
  float CompressedRelationWriter::computeMultiplicity(
    ...
    float multiplicity = functional ? 1.0f
                                    : static_cast<float>(numElements) /
                                          float(numDistinctElements);
  ...
  DecompressedBlock CompressedRelationReader::decompressBlock(
    ...
    return locatedTriples.mergeTriples(blockIndex, decompressedBlock,
  ...
  auto CompressedRelationReader::getFirstAndLastTriple(
      ...
      [[maybe_unused]] const LocatedTriplesPerBlock& locatedTriplesPerBlock)
      const -> std::optional<ScanSpecAndBlocksAndBounds::FirstAndLastTriple> {
    auto relevantBlocks = getBlocksFromMetadata(metadataAndBlocks);
  ...
  CompressedRelationMetadata CompressedRelationWriter::finishLargeRelation(
    ...
    md =
        CompressedRelationMetadata{currentCol0Id_, currentRelationPreviousSize_,
  ...
  auto CompressedRelationWriter::createPermutationPair(
    ...
    auto addBlockOfSmallRelationsToSwitched =
        [&writer2](std::shared_ptr<IdTable> relationPtr) {
          ...
          // Note: We could also use `compareWithoutLocalVocab` to compare the
          // IDs cheaper, but this sort is far from being a performance
          // bottleneck.
          auto compare = [](const auto& a, const auto& b) {
  ...
  CompressedRelationReader::getMetadataForSmallRelation(
    ...
    metadata.multiplicityCol1_ = CompressedRelationWriter::computeMultiplicity(
        block.size(), numDistinct);
