...
CompressedRelationReader::asyncParallelBlockGenerator(
    ...
    size_t numIndexColumns, LimitOffsetClause& limitOffset,
  ...
  auto readAndDecompressBlock = [&]()
    ...
    CompressedBlock compressedBlock = readCompressedBlockFromFile(
        blockMetadata,
        (locatedTriplesPerBlock.hasUpdates(blockMetadata.blockIndex_)
             ? columnIndicesWithAllIndex
             : columnIndices));
    lock.unlock();
    DecompressedBlock decompressedBlock =
        decompressBlock(compressedBlock, blockMetadata.numRows_);
    if (locatedTriplesPerBlock.hasUpdates(blockMetadata.blockIndex_)) {
      decompressedBlock =
...
CompressedRelationReader::IdTableGenerator CompressedRelationReader::lazyScan(
  ...
  if (beginBlockMetadata + 1 < endBlockMetadata) {
    ...
    auto blockGenerator = asyncParallelBlockGenerator(
        ...
        cancellationHandle, locatedTriplesPerBlock, numIndexColumns,
        limitOffset, filterDuplicatesAndGraphs);
