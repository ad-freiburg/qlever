...
// original vocabulary indices, so that `Id`s that refer to them stay valid; the
// result therefore is a vocabulary with holes (see
// `VocabularyInMemoryBinSearch`), and looking up an index that was excluded
// yields `ad_utility::vocabulary::placeholderForMissingVocabIndex`.
//
