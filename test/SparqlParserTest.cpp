...
TEST(ParserTest, testParse) {
  try {
...

    {
      ...
      ASSERT_EQ(std::nullopt, pq._limitOffset._offset);
    }
    ...
    */
...

    {
      ...
      ASSERT_EQ(std::nullopt, parsed_sub_query._subquery._limitOffset._offset);
      ...
      ASSERT_EQ(std::nullopt, parsed_sub_query._subquery._limitOffset._limit);
...

    {
      ...
      ASSERT_EQ(std::nullopt, aux_parsed_sub_sub_query._limitOffset._offset);
...
TEST(ParserTest, testSolutionModifiers) {
...

  {
    ...
    ASSERT_EQ(1u, selectClause._varsOrAsterisk.getSelectedVariables().size());
    ASSERT_EQ(1u, c._whereClauseTriples.size());
    ASSERT_EQ(10u, pq._limitOffset._limit.value_or(0));
    ASSERT_EQ(15u, pq._limitOffset._offset.value_or(0));
    ASSERT_EQ(size_t(0), pq._orderBy.size());
    ASSERT_FALSE(selectClause._distinct);
    ASSERT_FALSE(selectClause._reduced);
  }
