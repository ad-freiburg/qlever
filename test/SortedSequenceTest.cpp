...
// Shared test suite for the two implementations of `SortedSequence` (see
// `util/SortedSequence.h`): every test in this file is a typed test that runs
// against both `SortedSequenceTwoSortedRanges` and
// `SortedSequenceManySortedBlocks`, which enforces that the two
// implementations behave identically. Tests that are specific to one
// implementation (e.g. of its internal state) live in the respective
// `SortedSequence<Implementation>Test.cpp`.
...
// The two implementations under test. For the block-based implementation, a
// small block size is used so that the tests also exercise block splitting.
using TwoSortedRanges =
    SortedSequenceTwoSortedRanges<Pair, std::less<>,
                                  MemberProjection<&Pair::first>>;
using ManySortedBlocks =
    SortedSequenceManySortedBlocks<Pair, std::less<>,
                                   MemberProjection<&Pair::first>, 4>;
using Implementations = ::testing::Types<TwoSortedRanges, ManySortedBlocks>;

template <typename T>
struct SortedSequenceTypedTest : public ::testing::Test {};
TYPED_TEST_SUITE(SortedSequenceTypedTest, Implementations);
...
TYPED_TEST(SortedSequenceTypedTest, emptyAndClear) {
  TypeParam s;
  EXPECT_TRUE(s.empty());
...
// _____________________________________________________________________________
TYPED_TEST(SortedSequenceTypedTest, insertConsolidateAndDeduplicate) {
  TypeParam s;
...
// _____________________________________________________________________________
TYPED_TEST(SortedSequenceTypedTest, manyConsolidationRounds) {
  // Insert 20 keys in descending order across several consolidation rounds
  // (for the block-based implementation with block size 4, this exercises
  // block splitting).
  TypeParam s;
...
// _____________________________________________________________________________
TYPED_TEST(SortedSequenceTypedTest, fromSorted) {
  std::vector<Pair> sorted;
  for (int i = 0; i < 11; ++i) {
    sorted.push_back({i, i});
  }
  auto s = TypeParam::fromSorted(sorted);
...
// _____________________________________________________________________________
TYPED_TEST(SortedSequenceTypedTest, frontAndBack) {
  TypeParam s;
...
// _____________________________________________________________________________
TYPED_TEST(SortedSequenceTypedTest, eraseSingle) {
  TypeParam s;
...
// _____________________________________________________________________________
TYPED_TEST(SortedSequenceTypedTest, eraseSortedAndUnsorted) {
  TypeParam s;
...
// _____________________________________________________________________________
TYPED_TEST(SortedSequenceTypedTest, contractChecks) {
  TypeParam s;
  s.insert({1, 0});
...
TYPED_TEST(SortedSequenceTypedTest, randomizedAgainstReference) {
  std::mt19937 gen{42};
