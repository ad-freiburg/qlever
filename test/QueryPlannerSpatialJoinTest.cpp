...
TEST(QueryPlanner, SpatialJoinFromFilterWithFixedValue) {
  ...
  auto algo = SpatialJoinAlgorithm::LIBSPATIALJOIN;
  using enum SpatialJoinType::Enum;
  // The first (and, for these single-filter queries, only) internal variable
  // generated by the query planner while seeding filter substitutes.
  Var internalVar{
  ...
  // `geof:sfIntersects` with a fixed geometry on the right-hand side. With
  // only a single triple, there is no cartesian product for the `SpatialJoin`
  // substitute to save over a plain `FILTER`, so which of the two is cheaper
  // is a genuine cost-model choice; either is a correct plan.
  h::expect(
      absl::StrCat("PREFIX geof: "
                   ...
                   point, ")) }"),
      ::testing::AnyOf(h::spatialJoinFilterSubstitute(
                           -1, -1, Var{"?b"}, internalVar, std::nullopt,
                           PayloadVariables::all(), algo, INTERSECTS,
                           std::nullopt, scan("?a", "<p>", "?b"), anyValues),
                       h::Filter("sfIntersects", scan("?a", "<p>", "?b"))));

  // `geof:metricDistance(...) <= constant` with a fixed geometry on the
  // left-hand side. Same cost-model caveat as above applies.
  h::expect(
      absl::StrCat("PREFIX geof: "
                   "<http://www.opengis.net/def/function/geosparql/> "
                   "SELECT * WHERE {"
      ...
      ::testing::AnyOf(h::spatialJoinFilterSubstitute(
                           ...
                           std::nullopt, anyValues, scan("?a", "<p>", "?b")),
                       h::Filter("metricDistance", scan("?a", "<p>", "?b"))));
