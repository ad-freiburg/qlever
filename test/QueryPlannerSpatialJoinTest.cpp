...
// A `QueryPlanner` that runs the real `seedFilterSubstitutes` logic, but
// additionally records every generated `FiltersAndOptionalSubstitutes`
// result. Used to assert on the filter substitute candidates that the
// planner generates directly, independent of which (possibly
// cost-equivalent) candidate is ultimately picked by the cost-based planner.
class QueryPlannerFilterSubstituteSpy : public QueryPlanner {
  using QueryPlanner::QueryPlanner;

 public:
  std::vector<FiltersAndOptionalSubstitutes> seeded_;

  FiltersAndOptionalSubstitutes seedFilterSubstitutes(
      const std::vector<SparqlFilter>& filters) override {
    seeded_.push_back(QueryPlanner::seedFilterSubstitutes(filters));
    return seeded_.back();
  }
};
}  // namespace
...
TEST(QueryPlanner, SpatialJoinService) {
  ...
  h::expect(
      ...
      h::spatialJoin(100, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload,
                     BBox, std::nullopt, std::nullopt, scan("?x", "<p>", "?y"),
                     scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(
          -1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
          SpatialJoinType::DE9IM,
          De9imFilterString{'2', 'F', 'F', 'F', '1', 'F', 'F', 'F', '2'},
          scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(-1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
                     SpatialJoinType::COVERS, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(-1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
                     SpatialJoinType::CONTAINS, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(-1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
                     SpatialJoinType::TOUCHES, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(-1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
                     SpatialJoinType::CROSSES, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(-1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
                     SpatialJoinType::OVERLAPS, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(-1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
                     SpatialJoinType::WITHIN, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  h::expect(
      ...
      "{ ?a <p> ?b } }}",
      h::spatialJoin(-1, -1, V{"?y"}, V{"?b"}, std::nullopt, emptyPayload, SJ,
                     SpatialJoinType::EQUALS, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
...
TEST(QueryPlanner, SpatialJoinServiceMaxDistOutside) {
  ...
  h::expect(
      ...
      h::spatialJoin(1, -1, V{"?y"}, V{"?b"}, std::nullopt,
                     ...
                     // in this case
                     PayloadVariables::all(), S2, std::nullopt, std::nullopt,
                     scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  ...
  // Requested query for right child correctly pinned
  {
    ...
    h::expect(
        ...
        h::spatialJoin(500, -1, V{"?y"}, V{"?o"}, std::nullopt, PV::all(),
                       S2_POINT_POLYLINE, std::nullopt, std::nullopt,
                       scan("?x", "<p>", "?y"),
...
TEST(QueryPlanner, SpatialJoinFromGeofDistanceFilter) {
  ...
  h::expect(
      ...
      " }",
      h::spatialJoinFilterSubstitute(500, -1, V{"?y"}, V{"?b"}, std::nullopt,
  ...
  h::expect(
      ...
      h::spatialJoinFilterSubstitute(
          ...
          algo, type, std::nullopt, scan("?x", "<p>", "?y"),
          scan("?a", "<p>", "?b")));
  h::expect(
      "PREFIX geof: <http://www.opengis.net/def/function/geosparql/> "
      "SELECT * WHERE {"
  ...
  h::expect(
      ...
      ::testing::AnyOf(
          h::spatialJoinFilterSubstitute(
              ...
              algo, type, std::nullopt,
              h::spatialJoinFilterSubstitute(
                  500, -1, V{"?y"}, V{"?b"}, std::nullopt,
                  PayloadVariables::all(), algo, type, std::nullopt,
                  scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")),
              scan("?m", "<p>", "?n")),
          h::spatialJoinFilterSubstitute(
  ...
  h::expect(
      ...
      ::testing::AnyOf(
          ...
          h::spatialJoinFilterSubstitute(
              500, -1, V{"?y"}, V{"?b"},
              std::nullopt, PayloadVariables::all(), algo, type, std::nullopt,
              h::Bind(h::spatialJoinFilterSubstitute(
                          1000, -1, V{"?y"}, V{"?n"}, std::nullopt,
                          PayloadVariables::all(), algo, type, std::nullopt,
                          scan("?x", "<p>", "?y"), scan("?m", "<p>", "?n")),
...
TEST(QueryPlanner, SpatialJoinFromGeofRelationFilter) {
  ...
  for (const auto& [funcName, sjType] : geofFunctionNameAndSJType) {
    ...
    h::expect(query, h::spatialJoinFilterSubstitute(
                         ...
                         PayloadVariables::all(), algo, sjType, std::nullopt,
                         scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")));
  }
    ...
    h::expect(query,
              ::testing::AnyOf(
                  h::spatialJoinFilterSubstitute(
                      ...
                      PayloadVariables::all(), algo, sjType2, std::nullopt,
                      h::spatialJoinFilterSubstitute(
                          -1, -1, V{"?y"}, V{"?b"}, std::nullopt,
                          PayloadVariables::all(), algo, sjType1, std::nullopt,
                          scan("?x", "<p>", "?y"), scan("?a", "<p>", "?b")),
                      scan("?m", "<p>", "?n")),
                  h::spatialJoinFilterSubstitute(
                      -1, -1, V{"?y"}, V{"?b"}, std::nullopt,
      ...
      ::testing::AnyOf(
          h::Filter("geof:sfCovers(?n, ?b)",
                    h::spatialJoinFilterSubstitute(
                        ...
                        PayloadVariables::all(), algo, CONTAINS, std::nullopt,
                        scan("?m", "<p>", "?n"), scan("?a", "<p>", "?b"))),
          h::Filter("geof:sfContains(?n, ?b)",
                    h::spatialJoinFilterSubstitute(
                        -1, -1, V{"?n"}, V{"?b"}, std::nullopt,
                        PayloadVariables::all(), algo, COVERS, std::nullopt,
                        scan("?m", "<p>", "?n"), scan("?a", "<p>", "?b")))));
  ...
  h::expect(
      ...
      ::testing::AnyOf(
          ...
          h::spatialJoinFilterSubstitute(
              ...
              algo, CONTAINS, std::nullopt, scan("?m", "<p>", "?n"),
              h::spatialJoinFilterSubstitute(
...
TEST(QueryPlanner, SpatialJoinFromFilterWithFixedValue) {
  ...
  auto algo = SpatialJoinAlgorithm::LIBSPATIALJOIN;
  using enum SpatialJoinType::Enum;
  // The internal variable generated by the query planner while seeding the
  // filter substitute that ends up in the final plan. The planner may seed
  // filter substitutes more than once while planning a query (e.g. while
  // estimating the cost of falling back to greedy planning), so this is not
  // necessarily the first internal variable ever generated; we don't rely on
  // its exact index beyond matching what the planner actually produced.
  Var internalVar{
      absl::StrCat(QLEVER_INTERNAL_VARIABLE_QUERY_PLANNER_PREFIX, 1)};
  // The literal on the fixed side is resolved to `internalVar` via a one-row
  // `VALUES` clause instead of a second `IndexScan`; we don't care about its
  // exact cache key here.
  auto anyValues = h::RootOperation<::Values>(::testing::_);
  std::string point =
      "\"POINT(1 1)\"^^<http://www.opengis.net/ont/geosparql#wktLiteral>";

  // `geof:sfIntersects` with a fixed geometry on the right-hand side.
  // `sfIntersects` (like the other `sfXxx` relations and `geof:relate`) has
  // no standalone implementation as a plain `FILTER` (see
  // `WktGeometricRelation`), so the `SpatialJoin` substitute must always be
  // used, regardless of the cost-model comparison that applies to e.g.
  // `geof:metricDistance` below.
  h::expect(
      ...
      h::spatialJoinFilterSubstitute(
          ...
          algo, INTERSECTS, std::nullopt, scan("?a", "<p>", "?b"), anyValues));

  // `geof:sfContains` with a fixed geometry on either side: also mandatory,
  // like `sfIntersects` above, regardless of which side is fixed.
  h::expect(
      "PREFIX geof: <http://www.opengis.net/def/function/geosparql/> "
      "SELECT * WHERE {"
      "?a <p> ?b ."
      ...
      h::spatialJoinFilterSubstitute(
          -1, -1, Var{"?b"}, internalVar, std::nullopt, PayloadVariables::all(),
          algo, CONTAINS, std::nullopt, scan("?a", "<p>", "?b"), anyValues));
  ...
  h::expect(
      ...
      h::spatialJoinFilterSubstitute(
          -1, -1, internalVar, Var{"?b"}, std::nullopt, PayloadVariables::all(),
          algo, CONTAINS, std::nullopt, anyValues, scan("?a", "<p>", "?b")));

  // `geof:metricDistance(...) <= constant` with a fixed geometry on the
  // left-hand side: mandatory just like the relation filters above.
  ...
  // the (fully mandatory) substitute -- no `FILTER` remains anywhere, since
  // the substitute alone already covers the filter.
  h::expect(absl::StrCat("PREFIX geof: "
                         "<http://www.opengis.net/def/function/geosparql/> "
                         "SELECT * WHERE {"
                         "?a <p> ?b ."
...
// _____________________________________________________________________________
TEST(QueryPlanner, SpatialJoinFromFilterWithFixedValueSubstituteIsGenerated) {
  // The `h::expect` checks above intentionally allow either the `SpatialJoin`
  // substitute or the plain `FILTER` to win the cost comparison, because for
  // a single triple there is no cartesian product to avoid, so either is a
  // correct plan. That means those checks alone would still pass even if the
  // new single-seed substitute path (see
  // `QueryPlanner::runDynamicProgrammingOnConnectedComponent`, the `numSeeds
  // < 2` case) were broken or removed entirely. This test instead inspects
  // the candidates generated by `seedFilterSubstitutes` directly, so the
  // substitute's existence and shape is verified regardless of which
  // candidate the cost-based planner ends up picking.
  auto algo = SpatialJoinAlgorithm::LIBSPATIALJOIN;
  using enum SpatialJoinType::Enum;
  Var internalVar{
  ...
  ParsedQuery pq = ad_utility::testing::parseQuery(
      absl::StrCat("PREFIX geof: "
                   "<http://www.opengis.net/def/function/geosparql/> "
                   "SELECT * WHERE {"
                   "?a <p> ?b ."
                   "FILTER(geof:sfIntersects(?b, ",
                   point, ")) }"));
  QueryPlannerFilterSubstituteSpy planner{
      qec, std::make_shared<ad_utility::CancellationHandle<>>()};
  planner.createExecutionTree(pq);

  // `seedFilterSubstitutes` may be invoked more than once while planning the
  // query (for example while estimating the cost of falling back to greedy
  // planning); every invocation must consistently produce the substitute.
  ASSERT_FALSE(planner.seeded_.empty());
  const auto& substitutes = planner.seeded_.front();
  ASSERT_EQ(substitutes.size(), 1u);
  ASSERT_TRUE(substitutes.front().hasSubstitute());
  for (const auto& seededSubstitutes : planner.seeded_) {
    ASSERT_EQ(seededSubstitutes.size(), 1u);
    ASSERT_TRUE(seededSubstitutes.front().hasSubstitute());
  }
  // At seeding time, only the fixed-value side's `VALUES` child is already
  // built; the variable side (`?b`) is attached later via the normal join
  // graph, exactly like the un-fixed side of a variable-to-variable
  // substitute (see `QueryPlanner::seedFilterSubstitutes`).
  EXPECT_THAT(
      *substitutes.front().substitute_.value()._qet,
      h::spatialJoinFilterSubstitute(
          -1, -1, Var{"?b"}, internalVar, std::nullopt, PayloadVariables::all(),
          algo, INTERSECTS, std::nullopt, anyValues));
}
