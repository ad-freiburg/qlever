...
TEST(ConfigManagerTest, HumanReadableAddValidator) {
  ...
  m.addValidator([](const int& num) { return num <= 100; },
                 "'numberInRange' is bigger than 100.", numberInRangeOption);
  m.addValidator([](const int& num) { return num > 49; },
                 "'numberInRange' is smaller than 50.", numberInRangeOption);
  ...
  m.addValidator(
      ...
      "More than one bool option was choosen.", boolOneOption, boolTwoOption,
      boolThreeOption);
  checkValidator(
      m,
      nlohmann::json::parse(
  ...
  auto doValidatorParameterNotInConfigManagerTest = []<typename T>() {
    ...
    */
    auto checkAddValidatorBehavior = [&validatorDummyFunction](
                                         ConfigManager& m,
                                         ConfigOptionProxy<T> validOption,
...
TEST(ConfigManagerTest, HumanReadableAddOptionValidator) {
  ...
  mFirstLetter.addOptionValidator(
      ...
      "One of the option names doesn't start with the letter d.",
      correctLetter);
  ASSERT_NO_THROW(
