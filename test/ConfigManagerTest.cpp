...
void doValidatorTest(auto addValidatorFunction,
  ...
  */
  auto testGeneratedValidatorsOfConfigManager = [&adjustVariantArgument]<typename... Ts>(
    ...
    for (size_t validatorNumber = variantStart; validatorNumber < variantEnd; validatorNumber++) {
      ...
      values.
      Because there is no strict evaluation order for validators, it is also impossible to say,
      which of the bool only validators will throw the exception, so we can't check for a specific
      validator either.
      Instead, we only check, if it was bool validator.
      */
  ...
  */
  ...
  /*
  Reseting `mValidatorSubValidatorOption1`, so that the validator, that was added via
  `doExceptionMessageTest`, not longer fails.

  We can not use an r-value nlohmann json object for this, because there is no fitting constructor.
  */
