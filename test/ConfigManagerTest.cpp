...
// Small test for the helper class `ConfigManager::ConfigurationDocValidatorAssignment`.
TEST(ConfigManagerTest, ConfigurationDocValidatorAssignment) {
  // How many instances of `ConfigOption`s and `ConfigManager`s should be added for the test?
  constexpr size_t NUM_CONFIG_OPTION = 1;
  constexpr size_t NUM_CONFIG_MANAGER = NUM_CONFIG_OPTION;

  // Generate a vector of dummy `ConfigOptionValidatorManager`. Note: They will not actually work.
  ...
  /*
  @brief Create a vector of key and `std::vector<ConfigOptionValidatorManager>` pairs. The size of
  `std::vector<ConfigOptionValidatorManager>` is random.

  @param keyFactory The constructor for the key elements. Should take no arguments and return a
  a fresh instance every time, it is invoked.
  ...
  */
  auto createKeyAndValidatorPairVector = [&generateDummyValidatorManager]<typename KeyFactory>(
    ...
    `ConfigOptionValidatorManager`s, that will be added for each key. (To make the test less
    uniform.)
    */
  ...
  /*
  Test if a vector, generated by `createKeyAndValidatorPairVector`, has entries in
  `ConfigurationDocValidatorAssignment`, as described by the vector.
  Note: This will be checked via identity, not equality.
  */
  auto testPairVector =
      []<typename T>(
        ...
        std::ranges::for_each(pairVector, [&assignment](const auto& pair) {
          ...
          // Is every entry in `expectedValidatorVector` contained in the assignment?
          ASSERT_TRUE(std::ranges::all_of(
              expectedValidatorVector,
              [&assignment](const auto& valMan) { return assignment.containsValue(valMan); }));
  ...
  /*
  Generate the `ConfigOption`, together with their validators. Note: We do not need working
  `ConfigOption`. As long as they exists, everythings fine.
  */
  ...
  /*
  Generate the `ConfigManager`, together with their validators. Note: We do not need working
  `ConfigManager`. As long as they exists, everythings fine.
  */
  ...
  // Check the behavior, if the key has nothing assigned to it and the validator was never assigned
  // to anything.
  bool b;
  ...
  ASSERT_FALSE(assignment.containsValue(notIncludedValidator));
}
