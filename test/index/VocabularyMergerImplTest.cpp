...
ItemVec::value_type makeEntry(std::string_view word, bool isExternalized,
                              uint64_t id, uint64_t geoSortKey = 0) {
  return {word, PartialVocabIndexWithExternalFlag{id, isExternalized},
          geoSortKey};
}

// Read back a file written by `writePartialVocabularyToFile` and return its
// contents as a vector of (word, isExternalized, id, geoSortKey) tuples.
std::vector<std::tuple<std::string, bool, uint64_t, uint64_t>> readBack(
    const std::string& fileName) {
  ad_utility::serialization::FileReadSerializer reader{fileName};
  std::vector<std::tuple<std::string, bool, uint64_t, uint64_t>> result;
  reader >> result;
  return result;
}
}  // namespace
...
TEST(IndexVocabularyMergerImpl, writePartialVocabularyToFile) {
  ...
  ItemVec els{
      ...
      makeEntry(bigWord, true, 99, 12345),
  };
  ...
  EXPECT_EQ(roundtrip.at(0), std::make_tuple(std::string{"\"alpha\""}, false,
                                             uint64_t{7}, uint64_t{0}));
  EXPECT_EQ(roundtrip.at(1), std::make_tuple(std::string{"\"beta\""}, true,
                                             uint64_t{42}, uint64_t{0}));
  EXPECT_EQ(roundtrip.at(2), std::make_tuple(std::string{"_:blank"}, false,
                                             uint64_t{0}, uint64_t{0}));
  EXPECT_EQ(roundtrip.at(3),
            std::make_tuple(bigWord, true, uint64_t{99}, uint64_t{12345}));
