...
TEST(IdColumnTest, idRefRoundtripsAndMirrorsApi) {
  for (Id id : sampleIds()) {
    ...
    EXPECT_EQ(constRef.getBits(), id.getBits());
    ...
    EXPECT_EQ(payload, other.getBits().payload_);
    EXPECT_EQ(datatype, other.getBits().datatype_);
  }
}
...
TEST(IdColumnTest, viewConstructionAccessAndDefaultState) {
  ...
  for (size_t i = 0; i < ids.size(); ++i) {
    EXPECT_EQ(view.rawPayloads()[i], ids.at(i).getBits().payload_);
    EXPECT_EQ(view.rawDatatypes()[i], ids.at(i).getBits().datatype_);
  }

  // Mutation through the mutable view is visible in the backing arrays.
  view[0] = Id::makeFromInt(-1);
  EXPECT_EQ(payloads.at(0), Id::makeFromInt(-1).getBits().payload_);
}
...
TEST(IdColumnTest, iteratorSupportsRangeBasedForAndSort) {
  ...
  ql::ranges::sort(mutableView, {}, [](Id id) { return id.getBits(); });
  std::vector<Id> sortedIds = ids;
  ql::ranges::sort(sortedIds, {}, [](Id id) { return id.getBits(); });
