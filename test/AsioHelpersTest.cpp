...
void checkStrandNotRunning(auto strand) {
  auto future = net::post(
      net::bind_executor(strand, std::packaged_task<void()>{[]() { return; }}));
...
TEST(AsioHelpers, deadlockingWithStrands) {
  Context ctx;
  [[maybe_unused]] static constexpr auto dummy =
      [](auto strand, Context& ctx) -> net::awaitable<void> {
    ctx.x_++;
    EXPECT_TRUE(strand.running_in_this_thread());
    co_return;
  };

  [[maybe_unused]] static constexpr auto a =
      [](auto strand, auto& ctx) -> net::awaitable<void> {
    for (size_t i = 0; i < 10'000; ++i) {
      co_await ad_utility::runAwaitableOnStrandAwaitable(strand,
                                                         dummy(strand, ctx));
    }
...
TEST(AsioHelpers, correctStrandScheduling) {
  ...
  static constexpr auto dummy =
      [](auto strand,
         [[maybe_unused]] auto wrongStrand) -> net::awaitable<void> {
    EXPECT_TRUE(strand.running_in_this_thread());
  ...
  auto scheduler = [](Context& ctx) -> net::awaitable<void> {
    ...
    co_await ad_utility::runAwaitableOnStrandAwaitable(
        ctx.strand2_, dummy(ctx.strand2_, ctx.strand1_));
    EXPECT_TRUE(ctx.strand1_.running_in_this_thread());
    EXPECT_FALSE(ctx.strand2_.running_in_this_thread());
  };
...
TEST(AsioHelpers, raceConditionCancellation) {
  ...
  using Op = decltype(net::co_spawn(ctx.strand1_,
                                    ad_utility::runAwaitableOnStrandAwaitable(
                                        ctx.strands_.at(0), increment(ctx, 0)),
                                    net::deferred));
  std::vector<Op> ops;
  // for (size_t i = 0; i < numValues; ++i) {
  for (size_t i = 0; i < 10; ++i) {
    ops.push_back(net::co_spawn(ctx.strand1_,
                                ad_utility::runAwaitableOnStrandAwaitable(
                                    ctx.strands_.at(i), increment(ctx, i)),
                                net::deferred));
    // ops.push_back(ad_utility::runAwaitableOnStrandAwaitable(ctx.strands_.at(i),
    // increment(ctx, i)));
  }
  auto group = net::experimental::make_parallel_group(std::move(ops));
  auto awaitAll = [](auto& group) -> net::awaitable<void> {
