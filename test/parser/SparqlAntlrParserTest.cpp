...
TEST(SparqlParser, propertyPaths) {
  ...
  expectPathOrVar("a:a*", WithLength(Iri("<http://www.example.com/a>"), 0, max),
                  {{"a", "<http://www.example.com/>"}});
  expectPathOrVar("a:a{1,3}",
                  WithLength(Iri("<http://www.example.com/a>"), 1, 3),
                  {{"a", "<http://www.example.com/>"}});
  expectPathOrVar("a:a{3}", WithLength(Iri("<http://www.example.com/a>"), 3, 3),
                  {{"a", "<http://www.example.com/>"}});
  expectPathOrVar("a:a{1,}",
                  WithLength(Iri("<http://www.example.com/a>"), 1, max),
                  {{"a", "<http://www.example.com/>"}});
  ...
  // Test a bigger example that contains everything.
  {
    PropertyPath expected = Alternative(
        {Sequence({
         ...
         WithLength(
             ...
             1, max),
         Sequence({WithLength(Sequence({Iri("<http://www.example.com/a/a>"),
                                        Iri("<http://www.example.com/b/b>"),
                                        Iri("<a/b/c>")}),
                              1, 4),
                   WithLength(Sequence({Iri("<http://www.example.com/a/a>"),
                                        Iri("<http://www.example.com/b/b>"),
                                        Iri("<a/b/c>")}),
                              3, 3)}),
    ...
    expectPathOrVar(
        "a:a/b:b*|c:c|(a:a/b:b/<a/b/c>)+|(a:a/b:b/<a/b/c>){1,4}/(a:a/b:b/<a/b/"
        "c>){3}|!a|!(^a|a|^a:a)",
        expected,
...
TEST(SparqlParser, SelectQuery) {
  ...
  expectSelectQuery(
      ...
      selectABarFooMatcher(m::Graphs{iri("<x>")}, m::Graphs{iri("<y>")}));
...
TEST(SparqlParser, AskQuery) {
  ...
  defaultGraphs.insert(iri("<x>"));
  Graphs namedGraphs;
  namedGraphs.insert(iri("<y>"));
  expectAskQuery(
...
TEST(SparqlParser, Query) {
  ...
  // Tests around DESCRIBE.
  {
    ...
    auto Iri = [](const auto& x) { return iri(x); };
...
TEST(SparqlParser, EncodedIriManagerUsage) {
  ...
  auto unencoded456 = PropertyPath::fromIri(iri("<http://example.org/456>"));
