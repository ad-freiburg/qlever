...
TEST(ParallelBuffer, ParallelFileBuffer) {
  ...
  buf.open(filename);
  std::vector<ParallelFileBuffer::BufferType> expected {{'a', 'b', 'c', 'd'}, {'e', 'f', 'g', 'h'}, {'i', 'j'}};
...
TEST(ParallelBuffer, ParallelBufferWithEndRegex) {
  ...
  {
  // The following regex is not found in the data, and the data is too large for
  // one block, so the parsing fails
  ...
  {
  // The same example but with a larger blocksize, s.t. the complete input fits into a single
  // block. In this case it is no error that the regex can never be found.
