...
TEST(FTSAlgorithmsTest, oneVarFilterAggScoresAndTakeTopKContexts) {
  ...
  auto [it, success] = fMap1.emplace(1, IdTable{1, alloc()});
  ASSERT_TRUE(success);
  ...
  {
    auto [it, suc] = fMap1.emplace(0, IdTable{1, alloc()});
    ASSERT_TRUE(suc);
  ...
  {
    auto [it, suc] = fMap4.emplace(0, IdTable{4, alloc()});
    ASSERT_TRUE(suc);
    auto& el = it->second;
  ...
  {
    auto [it, suc] = fMap4.emplace(2, IdTable(4, alloc()));
    ASSERT_TRUE(suc);
...
TEST(FTSAlgorithmsTest, multVarsFilterAggScoresAndTakeTopKContexts) {
  try {
    ...
    auto [it, suc] = fMap1.emplace(1, IdTable{1, alloc()});
    it->second.push_back({1});
