...
// _____________________________________________________________________________
// `removeIncompleteUtf8Character` cuts off a trailing character that is not
// completely present, and leaves everything else unchanged.
TEST(StringUtils, removeIncompleteUtf8Character) {
  using ad_utility::removeIncompleteUtf8Character;
  ...
  for (std::string_view complete :
    ...
    EXPECT_EQ(removeIncompleteUtf8Character(complete), complete) << complete;
  }
  // 'ö' is `0xC3 0xB6`, '日' is `0xE6 0x97 0xA5`, and U+1F600 is
  // `0xF0 0x9F 0x98 0x80`, so all the proper prefixes of those characters are
  // removed.
  EXPECT_EQ(removeIncompleteUtf8Character("Fl\xC3"), "Fl");
  EXPECT_EQ(removeIncompleteUtf8Character("\xE6"), "");
  EXPECT_EQ(removeIncompleteUtf8Character("ab\xE6\x97"), "ab");
  EXPECT_EQ(removeIncompleteUtf8Character("ab\xF0\x9F\x98"), "ab");
  // A byte that is not a valid lead byte is removed together with the
  // continuation bytes that follow it.
  EXPECT_EQ(removeIncompleteUtf8Character("ab\xF8\x80"), "ab");
  // A string that consists of continuation bytes only has no lead byte at all,
  // so there is nothing that could be removed.
  EXPECT_EQ(removeIncompleteUtf8Character("\x80\x80"), "\x80\x80");
}
