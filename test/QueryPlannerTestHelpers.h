...
void expectWithGivenBudget(std::string query, auto matcher,
  ...
  auto budgetBackup = RuntimeParameters().get<QueryPlanningBudget>();
  RuntimeParameters().set<QueryPlanningBudget>(queryPlanningBudget);
  auto cleanup = absl::Cleanup{[budgetBackup]() {
    RuntimeParameters().set<QueryPlanningBudget>(budgetBackup);
  }};
