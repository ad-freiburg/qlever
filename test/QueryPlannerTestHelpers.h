...
void expectWithGivenBudget(std::string query, auto matcher,
  ...
  auto budgetBackup = runtimeParametersNew().rlock()->queryPlanningBudget.get();
  runtimeParametersNew().wlock()->queryPlanningBudget.set(queryPlanningBudget);
  auto cleanup = absl::Cleanup{[budgetBackup]() {
    runtimeParametersNew().wlock()->queryPlanningBudget.set(budgetBackup);
  }};
