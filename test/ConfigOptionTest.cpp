...
void doValidatorWithSingleParameterTypeOutOfListConceptTest(
  ...
  static_assert(
      !conceptFunction.template
       operator()<SingleIntValidatorFunction, std::string, ConfigOption,
...
TEST(ConfigOptionTest, AddValidator) {
  ...
  */
  auto createDummyValue = []<typename Type>(size_t variant,
                                            auto&& createDummyValue) -> Type {
    if constexpr (std::is_same_v<Type, bool>) {
      ...
      for (size_t i = 0; i <= variant; i++) {
        vec.push_back(
            createDummyValue.template operator()<typename Type::value_type>(
                i, createDummyValue));
      }
      return vec;
    }
  ...
  auto generateValidatorFunction = [&createDummyValue]<typename ParameterType>(
    ...
    if constexpr (std::is_same_v<ParameterType, bool>) {
      ...
      return [compareTo = createDummyValue.template operator()<ParameterType>(
                  variant, createDummyValue)](const bool& b) {
        return compareTo == b;
      };
    } else if constexpr (std::is_same_v<ParameterType, std::string>) {
      // Is the generated string a prefix of the given string?
      return [compareTo = createDummyValue.template operator()<ParameterType>(
                  variant, createDummyValue)](const std::string& s) {
        return s.starts_with(compareTo);
      };
      ...
      return [compareTo = createDummyValue.template operator()<ParameterType>(
                  variant, createDummyValue)](const ParameterType& n) {
        return n >= compareTo;
      };
    } else {
      // Must be a vector and we can go recursive.
      ...
      return [&generateValidatorFunction, variant](const ParameterType& v) {
        ...
        for (size_t i = 0; i < variant + 1; i++) {
          if (!generateValidatorFunction
                   .template operator()<typename ParameterType::value_type>(
  ...
  */
  ...
  // executing a test.
  auto doTest = [&createDummyValue, &generateValidatorFunction,
                 &checkSet]<typename Type>() {
    Type var{};
    ConfigOption option("Test", "", &var);

    // Single validator.
    option.addValidator(
        generateValidatorFunction.template operator()<Type>(
            1, generateValidatorFunction),
    ...
    checkSet(option,
             createDummyValue.template operator()<Type>(1, createDummyValue),
      ...
      */
      if constexpr (std::is_same_v<Type, bool>) {
        ...
        for (size_t validatorNumber = 1; validatorNumber <= i;
          ...
          checkSet(
              option,
              createDummyValue.template operator()<Type>(i, createDummyValue),
              createDummyValue.template operator()<Type>(validatorNumber - 1,
                                                         createDummyValue),
              absl::StrCat(ConfigOption::availableTypesToString<Type>(),
