...
TEST(ConfigOptionTest, ConstructorException) {
  ...
  // Names with spaces.
  ASSERT_ANY_THROW(
      ad_utility::makeConfigOption<bool>("Option 1", "", &notUsed));
}
...
static void doForTypeInValueType(Function function) {
  ad_utility::ConstexprForLoop(
      ...
      [&function]<size_t index,
                  typename IndexType = std::variant_alternative_t<
                      index, ConfigOption::ValueType>::value_type>() {
...
TEST(ConfigOptionTest, CreateSetAndTest) {
  ...
  */
  auto setAndTest = [&otherGettersDontWork]<typename Type>(
    ...
    ASSERT_EQ(ConfigOption::ValueType{std::optional<Type>{}}.index(),
  ...
  */
  auto testCaseWithDefault =
      [&setAndTest, &otherGettersDontWork]<typename Type>(
  ...
  auto testCaseWithoutDefault =
      [&setAndTest]<typename Type>(const Type& valueToSetTo) {
        // Every configuration option keeps updating an external variable with
        // the value, that it itself holds. This is the one.
        Type configurationOptionValue;

        ConfigOption option{ad_utility::makeConfigOption<Type>(
            "Without_default", "", &configurationOptionValue)};

        // Make sure, that we truly don't have a value, that can be gotten.
        ASSERT_TRUE(!option.hasValue() && !option.hasDefaultValue());
        ad_utility::ConstexprForLoop(
            std::make_index_sequence<
                std::variant_size_v<ConfigOption::ValueType>>{},
            [&option]<size_t index,
                      typename IndexType = std::variant_alternative_t<
                          index, ConfigOption::ValueType>::value_type>() {
  ...
  testCaseWithDefault.template operator()<std::string>(std::string{"unset"},
                                                       std::string{"set"});
  ...
  testCaseWithDefault.template operator()<std::vector<bool>>(
      std::vector{false, true}, {true, true});
  testCaseWithoutDefault.template operator()<std::vector<bool>>(
  ...
  testCaseWithDefault.template operator()<std::vector<int>>(
      std::vector{40, 42}, std::vector{42, 42});
  testCaseWithoutDefault.template operator()<std::vector<int>>(
      std::vector{42, 42});

  testCaseWithDefault.template operator()<std::vector<float>>(
      std::vector<float>{40.8, 42.8}, std::vector<float>{42.8, 42.8});
  testCaseWithoutDefault.template operator()<std::vector<float>>(
...
TEST(ConfigOptionTest, SetValueWithJson) {
  ...
  auto getTestCase = []<typename Type>() {
    if constexpr (std::is_same_v<Type, bool>) {
      ...
      return JsonTestCase<std::vector<float>>{
          std::vector<float>{42.8, 42.8},
  ...
  */
  auto doTestCase = [&getTestCase]<typename Type>() {
    ...
    ConfigOption option{
        ad_utility::makeConfigOption<Type>("t", "", &configurationOptionValue)};
    ...
    doForTypeInValueType([&getTestCase, &option]<typename CurrentType>() {
      if constexpr (!std::is_same_v<Type, CurrentType>) {
        ASSERT_ANY_THROW(option.setValueWithJson(
            getTestCase.template operator()<CurrentType>().json););
      }
    });
...
TEST(ConfigOptionTest, Visit) {
  ...
  equal
  */
  auto createComparisonVisitor =
      []<typename ComparatorType>(const ComparatorType& comparator) {
  ...
  correct value.
  */
  auto setAndTest = [&createComparisonVisitor]<typename Type>(
                        ConfigOption& option, const Type& valueToSetTo) {
    option.setValue(valueToSetTo);
  ...
  */
  auto testValues = []<typename Type>() {
    if constexpr (std::is_same_v<Type, bool>) {
      ...
      return std::make_pair(
          std::vector{std::string{"str1"}, std::string{"str1"}},
          std::vector{std::string{"str2"}, std::string{"str2"}});
    } else if constexpr (std::is_same_v<Type, std::vector<int>>) {
      return std::make_pair(std::vector<int>{42, 42}, std::vector<int>{44, 43});
    } else if constexpr (std::is_same_v<Type, std::vector<float>>) {
      return std::make_pair(std::vector<float>{42.8, 42.8},
                            std::vector<float>{52.8, 42.9});
    }
  };
  ...
  setting it. With or without a default value.
  */
  auto testCaseWithDefault = [&createComparisonVisitor, &setAndTest,
                              &testValues]<typename Type>() {
  ...
  auto testCaseWithoutDefault = [&setAndTest, &testValues]<typename Type>() {
    ...
    option.visitDefaultValue(
        [](const auto& val) { ASSERT_FALSE(val.has_value()); });
