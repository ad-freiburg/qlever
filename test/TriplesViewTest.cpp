...
struct DummyPermutation {
  void scan(Id col0Id, auto* result) const {
    ...
    for (size_t i = 0; i < col0Id.getVocabUnchecked(); ++i) {
      result->push_back(std::array{V((i + 1) * col0Id.getVocabUnchecked()), V((i + 2) * col0Id.getVocabUnchecked())});
    }
  }
...
TEST(TriplesView, IgnoreRanges) {
  ...
  std::vector<std::pair<Id, Id>> ignoredRanges{{V(0), V(4)}, {V(7), V(8)}, {V(13), V(87593)}};
  for (auto triple :
...
TEST(TriplesView, IgnoreTriples) {
  std::vector<std::array<Id, 3>> result;
  auto expected = expectedResult();
  auto isTripleIgnored = [](const auto& triple) { return triple[1].getVocabUnchecked() % 2 == 0; };
