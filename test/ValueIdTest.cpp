...
TEST_F(ValueIdTest, Indices) {
  ...
  testRandomIds(&makeWordVocabId, &getWordVocabIndex, Datatype::WordVocabIndex,
                indexGenerator, ValueId::maxIndex);
  // Unlike the other index types, `SecondaryVocabIndex` still enforces
  // `maxSecondaryVocabIndex` (see `ValueId.h`).
  testRandomIds(&makeSecondaryVocabId, &getSecondaryVocabIndex,
                Datatype::SecondaryVocabIndex, indexGenerator,
                ValueId::maxSecondaryVocabIndex);
}
...
TEST_F(ValueIdTest, IndexOrdering) {
  ...
  {
    // Unlike the other index types, `SecondaryVocabIndex` still enforces
    // `maxSecondaryVocabIndex` (see `ValueId.h`), so it needs a bounded
    // generator.
    std::vector<ValueId> ids;
    addIdsFromGenerator(secondaryVocabIndexGenerator, &makeSecondaryVocabId,
                        ids);
    std::vector<uint64_t> indices;
    for (auto id : ids) {
      indices.push_back(getSecondaryVocabIndex(id));
    }
    std::sort(ids.begin(), ids.end());
    std::sort(indices.begin(), indices.end());
    for (size_t i = 0; i < ids.size(); ++i) {
      ASSERT_EQ(getSecondaryVocabIndex(ids[i]), indices[i]);
    }
  }
