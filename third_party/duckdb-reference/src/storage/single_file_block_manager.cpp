...
void SerializeVersionNumber(WriteStream& ser, const string& version_str) {
  ...
  memcpy(version, version_str.c_str(),
         MinValue<idx_t>(version_str.size(), MainHeader::MAX_VERSION_SIZE));
...
void MainHeader::CheckMagicBytes(FileHandle& handle) {
  ...
  if (handle.GetFileSize() <
      MainHeader::MAGIC_BYTE_SIZE + MainHeader::MAGIC_BYTE_OFFSET) {
    throw IOException(
        "The file \"%s\" exists, but it is not a valid DuckDB database file!",
        handle.path);
  }
  handle.Read(magic_bytes, MainHeader::MAGIC_BYTE_SIZE,
              MainHeader::MAGIC_BYTE_OFFSET);
  if (memcmp(magic_bytes, MainHeader::MAGIC_BYTES,
...
MainHeader MainHeader::Read(ReadStream& source) {
  ...
  if (header.version_number < VERSION_NUMBER_LOWER ||
    ...
    if (!version.empty()) {
      ...
      version_text =
          string("an ") +
          (VERSION_NUMBER_UPPER > header.version_number ? "older development"
                                                        : "newer") +
          string(" version of DuckDB");
    }
    throw IOException(
        "Trying to read a database file with version number %lld, but we can "
        "only read versions between %lld and "
        "%lld.\n"
        "The database file was created with %s.\n\n"
        "Newer DuckDB version might introduce backward incompatible changes "
        "(possibly guarded by compatibility "
        "settings)"
...
DatabaseHeader DatabaseHeader::Read(const MainHeader& main_header,
                                    ReadStream& source) {
  ...
  if (header.vector_size != STANDARD_VECTOR_SIZE) {
    throw IOException(
        "Cannot read database file: DuckDB's compiled vector size is %llu "
        "bytes, but the file has a "
        "vector size of %llu bytes.",
        STANDARD_VECTOR_SIZE, header.vector_size);
  }
  if (main_header.version_number == 64) {
    // version number 64 does not have the serialization compatibility in the
    // file - default to 1
...
DatabaseHeader DeserializeDatabaseHeader(const MainHeader& main_header,
                                         data_ptr_t ptr) {
  MemoryStream source(ptr, Storage::FILE_HEADER_SIZE);
...
SingleFileBlockManager::SingleFileBlockManager(
    AttachedDatabase& db, const string& path_p,
    ...
    : BlockManager(BufferManager::GetBufferManager(db),
      ...
      path(path_p),
      header_buffer(
          Allocator::Get(db), FileBufferType::MANAGED_BUFFER,
...
FileOpenFlags SingleFileBlockManager::GetFileFlags(bool create_new) const {
  ...
  if (options.read_only) {
    D_ASSERT(!create_new);
    result = FileFlags::FILE_FLAGS_READ |
             FileFlags::FILE_FLAGS_NULL_IF_NOT_EXISTS | FileLockType::READ_LOCK;
  } else {
    result = FileFlags::FILE_FLAGS_WRITE | FileFlags::FILE_FLAGS_READ |
...
void SingleFileBlockManager::AddStorageVersionTag() {
  db.tags["storage_version"] =
      GetStorageVersionName(options.storage_version.GetIndex());
}
...
void SingleFileBlockManager::CreateNewDatabase() {
  ...
  MainHeader main_header =
      ConstructMainHeader(options.version_number.GetIndex());
  ...
  // initialize meta_block and free_list to INVALID_BLOCK because the database
  // file does not contain any actual content yet
  DatabaseHeader h1;
  // header 1
  h1.iteration = 0;
  ...
  h1.block_count = 0;
  // We create the SingleFileBlockManager with the desired block allocation size
  // before calling CreateNewDatabase.
...
void SingleFileBlockManager::LoadExistingDatabase() {
  ...
  handle = fs.OpenFile(path, flags);
  if (!handle) {
    // this can only happen in read-only mode - as that is when we set
    // FILE_FLAGS_NULL_IF_NOT_EXISTS
    throw IOException(
        "Cannot open database \"%s\" in read-only mode: database does not "
        "exist",
        path);
  }
...
void SingleFileBlockManager::ReadAndChecksum(FileBuffer& block,
  ...
  if (stored_checksum != computed_checksum) {
    throw IOException(
        "Corrupt database file: computed checksum %llu does not match stored "
        "checksum %llu in block "
        "at location %llu",
...
void SingleFileBlockManager::Initialize(const DatabaseHeader& header,
  ...
  if (options.storage_version.IsValid()) {
    ...
    if (requested_compat_version < header.serialization_compatibility) {
      throw InvalidInputException(
          "Error opening \"%s\": cannot initialize database with storage "
          "version %d - which is lower than what "
          "the database itself uses (%d). The storage version of an existing "
          "database cannot be lowered.",
  ...
  if (header.serialization_compatibility >
      SerializationCompatibility::Latest().serialization_version) {
    throw InvalidInputException(
        "Error opening \"%s\": file was written with a storage version greater "
        "than the latest version supported "
        "by this DuckDB instance. Try opening the file with a newer version of "
        "DuckDB.",
        path);
  }
  db.GetStorageManager().SetStorageVersion(options.storage_version.GetIndex());
  ...
  if (block_alloc_size.IsValid() &&
      block_alloc_size.GetIndex() != header.block_alloc_size) {
    throw InvalidInputException(
        "Error opening \"%s\": cannot initialize the same database with a "
        "different block size: provided block "
...
void SingleFileBlockManager::LoadFreeList() {
  ...
  MetadataReader reader(GetMetadataManager(), free_pointer, nullptr,
                        BlockReaderType::REGISTER_BLOCKS);
...
void SingleFileBlockManager::MarkBlockAsFree(block_id_t block_id) {
  ...
  if (free_list.find(block_id) != free_list.end()) {
    throw InternalException(
        "MarkBlockAsFree called but block %llu was already freed!", block_id);
  }
...
void SingleFileBlockManager::MarkBlockAsUsed(block_id_t block_id) {
  ...
  if (max_block <= block_id) {
    ...
    // i.e. if max_block = 0, and block_id = 3, we need to add blocks 1 and 2 to
    // the free list
...
void SingleFileBlockManager::VerifyBlocks(
    const unordered_map<block_id_t, idx_t>& block_usage_count) {
  // probably don't need this?
  lock_guard<mutex> lock(block_lock);
  // all blocks should be accounted for - either in the block_usage_count, or in
  // the free list
  ...
  for (auto& block : block_usage_count) {
    ...
    if (block.second > 1) {
      ...
      if (entry == multi_use_blocks.end()) {
        throw InternalException(
            "Block %lld was used %llu times, but not present in "
            "multi_use_blocks",
            block.first, block.second);
      }
      if (entry->second != block.second) {
        throw InternalException(
            "Block %lld was used %llu times, but multi_use_blocks says it is "
            "used %llu times",
            block.first, block.second, entry->second);
      }
    } else {
      ...
      if (entry != free_list.end()) {
        throw InternalException(
            "Block %lld was used, but it is present in the free list",
            block.first);
      }
    }
  }
  ...
  if (referenced_blocks.size() != NumericCast<idx_t>(max_block)) {
    ...
    throw InternalException(
        "Blocks %s were neither present in the free list or in the "
        "block_usage_count (max block %lld)",
...
bool SingleFileBlockManager::IsRemote() { return !handle->OnDiskFile(); }
...
void SingleFileBlockManager::Read(Block& block) {
  D_ASSERT(block.id >= 0);
  D_ASSERT(std::find(free_list.begin(), free_list.end(), block.id) ==
           free_list.end());
...
void SingleFileBlockManager::ReadBlocks(FileBuffer& buffer,
  ...
  for (idx_t i = 0; i < block_count; i++) {
    ...
    uint64_t computed_checksum = Checksum(
        start_ptr + Storage::DEFAULT_BLOCK_HEADER_SIZE, GetBlockSize());
    // verify the checksum
    if (stored_checksum != computed_checksum) {
      throw IOException(
          "Corrupt database file: computed checksum %llu does not match stored "
...
void SingleFileBlockManager::Write(FileBuffer& buffer, block_id_t block_id) {
  D_ASSERT(block_id >= 0);
  ChecksumAndWrite(
      buffer, BLOCK_START + NumericCast<idx_t>(block_id) * GetBlockAllocSize());
}
...
vector<MetadataHandle> SingleFileBlockManager::GetFreeListBlocks() {
  ...
  // since these blocks are no longer free we cannot just include them in the
  // free list!
  ...
  while (true) {
    auto free_list_size =
        sizeof(uint64_t) +
        sizeof(block_id_t) * (free_list.size() + modified_blocks.size());
    auto multi_use_blocks_size =
        sizeof(uint64_t) +
        (sizeof(block_id_t) + sizeof(uint32_t)) * multi_use_blocks.size();
    auto metadata_blocks =
        sizeof(uint64_t) + (sizeof(block_id_t) + sizeof(idx_t)) *
                               GetMetadataManager().BlockCount();
...
class FreeListBlockWriter : public MetadataWriter {
 public:
  FreeListBlockWriter(MetadataManager& manager,
                      vector<MetadataHandle> free_list_blocks_p)
      : MetadataWriter(manager),
        free_list_blocks(std::move(free_list_blocks_p)),
        index(0) {}
 ...
 protected:
  MetadataHandle NextHandle() override {
    if (index >= free_list_blocks.size()) {
      throw InternalException(
          "Free List Block Writer ran out of blocks, this means not enough "
          "blocks were allocated up front");
    }
...
void SingleFileBlockManager::WriteHeader(DatabaseHeader header) {
  ...
  if (!free_list_blocks.empty()) {
    // there are blocks to write, either in the free_list or in the
    // modified_blocks we write these blocks specifically to the
    // free_list_blocks a normal MetadataWriter will fetch blocks to use from
    // the free_list but since we are WRITING the free_list, this behavior is
    // sub-optimal
  ...
  if (config.options.checkpoint_abort ==
      CheckpointAbort::DEBUG_ABORT_AFTER_FREE_LIST_WRITE) {
    throw FatalException(
        "Checkpoint aborted after free list write because of PRAGMA "
        "checkpoint_abort flag");
  }

  // We need to fsync BEFORE we write the header to ensure that all the previous
  // blocks are written as well
  handle->Sync();

  header_buffer.Clear();
  // if we are upgrading the database from version 64 -> version 65, we need to
  // re-write the main header
  if (options.version_number.GetIndex() == 64 &&
      options.storage_version.GetIndex() >= 4) {
  ...
  // now write the header to the file, active_header determines whether we write
  // to h1 or h2 note that if active_header is h1 we write to h2, and vice versa
  ChecksumAndWrite(header_buffer, active_header == 1
                                      ? Storage::FILE_HEADER_SIZE
...
void SingleFileBlockManager::TrimFreeBlocks() {
  if (DBConfig::Get(db).options.trim_free_blocks) {
    for (auto itr = newly_freed_list.begin(); itr != newly_freed_list.end();
         ++itr) {
      ...
      // Trim the range.
      handle->Trim(
          BLOCK_START + (NumericCast<idx_t>(first) * GetBlockAllocSize()),
