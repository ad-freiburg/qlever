...
BlockHandle::BlockHandle(BlockManager& block_manager, block_id_t block_id_p,
    ...
    : block_manager(block_manager),
      ...
      buffer(nullptr),
      eviction_seq_num(0),
      destroy_buffer_upon(DestroyBufferUpon::BLOCK),
      memory_charge(tag, block_manager.buffer_manager.GetBufferPool()),
      unswizzled(nullptr),
...
BlockHandle::BlockHandle(BlockManager& block_manager, block_id_t block_id_p,
                         MemoryTag tag, unique_ptr<FileBuffer> buffer_p,
                         DestroyBufferUpon destroy_buffer_upon_p,
                         idx_t block_size, BufferPoolReservation&& reservation)
    : block_manager(block_manager),
      readers(0),
      block_id(block_id_p),
      tag(tag),
      buffer_type(buffer_p->GetBufferType()),
      eviction_seq_num(0),
...
unique_ptr<Block> AllocateBlock(BlockManager& block_manager,
...
void BlockHandle::MergeMemoryReservation(BlockLock& l,
...
void BlockHandle::ResizeBuffer(BlockLock& l, idx_t block_size,
  ...
  memory_usage = NumericCast<idx_t>(NumericCast<int64_t>(memory_usage.load()) +
                                    memory_delta);
  D_ASSERT(memory_usage == buffer->AllocSize());
}

BufferHandle BlockHandle::LoadFromBuffer(BlockLock& l, data_ptr_t data,
                                         unique_ptr<FileBuffer> reusable_buffer,
                                         BufferPoolReservation reservation) {
  VerifyMutex(l);
  ...
  auto block =
      AllocateBlock(block_manager, std::move(reusable_buffer), block_id);
  ...
  if (block_id < MAXIMUM_BLOCK) {
    ...
    if (MustWriteToTemporaryFile()) {
      buffer = block_manager.buffer_manager.ReadTemporaryBuffer(
          tag, *this, std::move(reusable_buffer));
    } else {
      return BufferHandle();  // Destroyed upon unpin/evict, so there is no temp
                              // buffer to read
    }
  }
  state = BlockState::BLOCK_LOADED;
...
unique_ptr<FileBuffer> BlockHandle::UnloadAndTakeBlock(BlockLock& lock) {
  ...
  if (block_id >= MAXIMUM_BLOCK && MustWriteToTemporaryFile()) {
    // temporary block that cannot be destroyed upon evict/unpin: write to
    // temporary file
...
void BlockHandle::ConvertToPersistent(BlockLock& l, BlockHandle& new_block,
