...
struct EvictionQueue {
 ...
 public:
  ...
  //! Tries to dequeue an element from the eviction queue, but only after
  //! acquiring the purge queue lock.
 ...
 private:
  //! Bulk purge dead nodes from the eviction queue. Then, enqueue those that
  //! are still alive.
 ...
 private:
  ...
  //! We multiply the purge size by this value to determine early-outs. This is
  //! the minimum queue size. We never purge below this point.
  constexpr static idx_t EARLY_OUT_MULTIPLIER = 4;
  //! We multiply the approximate alive nodes by this value to test whether our
  //! total dead nodes exceed their allowed ratio. Must be greater than 1.
  constexpr static idx_t ALIVE_NODE_MULTIPLIER = 4;

 private:
  //! Total number of insertions into the eviction queue. This guides the
  //! schedule for calling PurgeQueue.
  atomic<idx_t> evict_queue_insertions;
  //! Total dead nodes in the eviction queue. There are two scenarios in which a
  //! node dies: (1) we destroy its block handle, or (2) we insert a newer
  //! version into the eviction queue.
  atomic<idx_t> total_dead_nodes;

  //! Locked, if a queue purge is currently active or we're trying to forcefully
  //! evict a node. Only lets a single thread enter the purge phase.
  mutex purge_lock;
  //! A pre-allocated vector of eviction nodes. We reuse this to keep the
  //! allocation overhead of purges small.
...
void EvictionQueue::Purge() {
  ...
  // For most scenarios, purging INSERT_INTERVAL * PURGE_SIZE_MULTIPLIER nodes
  // is enough. Purging more nodes than we insert also counters oscillation for
  // scenarios where most nodes are dead. If we always purge slightly more, we
  // trigger a purge less often, as we purge below the trigger.

  // However, if the pressure on the queue becomes too contested, we need to
  // purge more aggressively, i.e., we actively seek a specific number of dead
  // nodes to purge. We use the total number of existing dead nodes. We detect
  // this situation by observing the queue's ratio between alive vs. dead nodes.
  // If the ratio of alive vs. dead nodes grows faster than we can purge, we
  // keep purging until we hit one of the following conditions.

  // 2.1. We're back at an approximate queue size less than purge_size *
  // EARLY_OUT_MULTIPLIER. 2.2. We're back at a ratio of
  // 1*alive_node:ALIVE_NODE_MULTIPLIER*dead_nodes. 2.3. We've purged the entire
  // queue: max_purges is zero. This is a worst-case scenario, guaranteeing that
  // we always exit the loop.
...
void EvictionQueue::PurgeIteration(const idx_t purge_size) {
  // if this purge is significantly smaller or bigger than the previous purge,
  // then we need to resize the purge_nodes vector. Note that this barely
  // happens, as we purge queue_insertions * PURGE_SIZE_MULTIPLIER nodes
  idx_t previous_purge_size = purge_nodes.size();
  if (purge_size < previous_purge_size / 2 ||
      purge_size > previous_purge_size) {
...
EvictionQueue& BufferPool::GetEvictionQueueForBlockHandle(
  ...
  // Adjust if eviction_queue_idx is set (idx == 0 -> add at back, idx >=
  // queue_size -> add at front)
...
BufferPool::EvictionResult BufferPool::EvictBlocks(
  ...
  // This can never happen since we always return when i == 1. Exception to
...
BufferPool::EvictionResult BufferPool::EvictBlocksInternal(
  ...
  if (memory_usage.GetUsedMemory(MemoryUsageCaches::NO_FLUSH) <= memory_limit) {
    if (Allocator::SupportsFlush() &&
        extra_memory > allocator_bulk_deallocation_flush_threshold) {
      Allocator::FlushAll();
    }
  ...
  if (!found) {
    r.Resize(0);
  } else if (Allocator::SupportsFlush() &&
             extra_memory > allocator_bulk_deallocation_flush_threshold) {
    Allocator::FlushAll();
  }
...
idx_t BufferPool::PurgeAgedBlocksInternal(EvictionQueue& queue,
                                          ...
                                          int64_t limit) {
  idx_t purged_bytes = 0;
  queue.IterateUnloadableBlocks([&](BufferEvictionNode& node,
                                    const shared_ptr<BlockHandle>& handle,
                                    BlockLock& lock) {
    // We will unload this block regardless. But stop the iteration immediately
    // afterward if this block is younger than the age threshold.
...
void EvictionQueue::IterateUnloadableBlocks(FN fn) {
  for (;;) {
    ...
    // we might be able to free this block: grab the mutex and check if we can
    // free it
...
void BufferPool::SetLimit(idx_t limit, const char* exception_postscript) {
  ...
  if (!EvictBlocks(MemoryTag::EXTENSION, 0, limit).success) {
    throw OutOfMemoryException(
        "Failed to change memory limit to %lld: could not free up enough "
...
void BufferPool::MemoryUsage::UpdateUsedMemory(MemoryTag tag, int64_t size) {
  ...
  if ((idx_t)AbsValue(size) < MEMORY_USAGE_CACHE_THRESHOLD) {
    ...
    auto cache_idx =
        (idx_t)TaskScheduler::GetEstimatedCPUId() % MEMORY_USAGE_CACHE_COUNT;
