...
static void WriteGarbageIntoBuffer(BlockLock& lock, BlockHandle& block) {
  ...
  memset(buffer->buffer, 0xa5,
         buffer->size);  // 0xa5 is default memory in debug mode
}
...
struct BufferAllocatorData : PrivateAllocatorData {
  explicit BufferAllocatorData(StandardBufferManager& manager)
      : manager(manager) {}
...
unique_ptr<FileBuffer> StandardBufferManager::ConstructManagedBuffer(
  ...
  if (source) {
    ...
    // no reusable buffer: allocate a new buffer
...
StandardBufferManager::StandardBufferManager(DatabaseInstance& db, string tmp)
    : BufferManager(),
      ...
      buffer_allocator(BufferAllocatorAllocate, BufferAllocatorFree,
...
TempBufferPoolReservation StandardBufferManager::EvictBlocksOrThrow(
  ...
  if (!r.success) {
    string extra_text = StringUtil::Format(
        " (%s/%s used)",
        StringUtil::BytesToHumanReadableString(GetUsedMemory()),
...
shared_ptr<BlockHandle> StandardBufferManager::RegisterTransientMemory(
    const idx_t size, const idx_t block_size) {
  D_ASSERT(size <= block_size);

  // This comparison is the reason behind passing block_size through transient
  // memory creation. Otherwise, any non-default block size would register as
  // small memory, causing problems when trying to convert that memory to
  // consistent blocks later on.
  if (size < block_size) {
...
shared_ptr<BlockHandle> StandardBufferManager::RegisterSmallMemory(
  ...
  auto reservation = EvictBlocksOrThrow(
      tag, size, nullptr, "could not allocate block of size %s%s",
...
void StandardBufferManager::BatchRead(vector<shared_ptr<BlockHandle>>& handles,
  ...
  if (block_count == 1) {
    // prefetching with block_count == 1 has no performance impact since we
    // can't batch reads skip the prefetch in this case we do it anyway if
    // alternative_verify is on for extra testing
    return;
  }
#endif

  // allocate a buffer to hold the data of all of the blocks
  auto intermediate_buffer = Allocate(
      MemoryTag::BASE_TABLE, block_count * block_manager.GetBlockSize());
  // perform a batch read of the blocks into the buffer
  block_manager.ReadBlocks(intermediate_buffer.GetFileBuffer(), first_block,
                           block_count);
  ...
  for (idx_t block_idx = 0; block_idx < block_count; block_idx++) {
    ...
    D_ASSERT(entry !=
             load_map.end());  // if we allow gaps we might not return true here
    ...
    // the prefetching relies on the block handle being pinned again during the
    // actual read before it is evicted
    BufferHandle buf;
    {
      auto lock = handle->GetLock();
      if (handle->GetState() == BlockState::BLOCK_LOADED) {
        // the block is loaded already by another thread - free up the
        // reservation and continue
...
BufferHandle StandardBufferManager::Pin(shared_ptr<BlockHandle>& handle) {
  // we need to be careful not to return the BufferHandle to this block while
  // holding the BlockHandle's lock as exiting this function's scope may cause
  // the destructor of the BufferHandle to be called while holding the lock the
  // destructor calls Unpin, which grabs the BlockHandle's lock again, causing a
  // deadlock
  BufferHandle buf;
  ...
  idx_t required_memory;
  {
    ...
    if (handle->GetState() == BlockState::BLOCK_LOADED) {
      // the block is loaded, increment the reader count and set the
      // BufferHandle
      buf = handle->Load();
    }
  ...
  if (buf.IsValid()) {
    return buf;  // the block was already loaded, return it without holding the
                 // BlockHandle's lock
  } else {
    ...
    // lock the handle again and repeat the check (in case anybody loaded in the
    // meantime)
    auto lock = handle->GetLock();
    // check if the block is already loaded
    if (handle->GetState() == BlockState::BLOCK_LOADED) {
      // the block is loaded, increment the reader count and return a pointer to
      // the handle
      reservation.Resize(0);
      buf = handle->Load();
    } else {
      ...
      // in the case of a variable sized block, the buffer may be smaller than a
      // full block.
      int64_t delta =
  ...
  // we should have a valid BufferHandle by now, either because the block was
  // already loaded, or because we loaded it return it without holding the
  // BlockHandle's lock
...
void StandardBufferManager::VerifyZeroReaders(BlockLock& lock,
                                              shared_ptr<BlockHandle>& handle) {
#ifdef DUCKDB_DEBUG_DESTROY_BLOCKS
  unique_ptr<FileBuffer> replacement_buffer;
  auto& allocator = Allocator::Get(db);
  auto alloc_size =
      handle->GetMemoryUsage() - Storage::DEFAULT_BLOCK_HEADER_SIZE;
  ...
  if (handle->GetBufferType() == FileBufferType::BLOCK) {
    ...
    replacement_buffer =
        make_uniq<FileBuffer>(allocator, buffer->GetBufferType(), alloc_size);
  }
...
unique_ptr<FileBuffer> StandardBufferManager::ReadTemporaryBufferInternal(
    BufferManager& buffer_manager, FileHandle& handle, idx_t position,
    idx_t size, unique_ptr<FileBuffer> reusable_buffer) {
  auto buffer =
      buffer_manager.ConstructManagedBuffer(size, std::move(reusable_buffer));
...
string StandardBufferManager::GetTemporaryPath(block_id_t id) {
  ...
  return fs.JoinPath(temporary_directory.path,
                     "duckdb_temp_block-" + to_string(id) + ".block");
}
...
void StandardBufferManager::RequireTemporaryDirectory() {
  if (temporary_directory.path.empty()) {
    throw InvalidInputException(
        ...
        "specified!\nTo enable "
        "temporary buffer eviction set a temporary directory using PRAGMA "
        "temp_directory='/path/to/tmp.tmp'");
  }
  lock_guard<mutex> guard(temporary_directory.lock);
  if (!temporary_directory.handle) {
    // temp directory has not been created yet: initialize it
    temporary_directory.handle = make_uniq<TemporaryDirectoryHandle>(
        db, temporary_directory.path, temporary_directory.maximum_swap_space);
  }
}
...
void StandardBufferManager::WriteTemporaryBuffer(MemoryTag tag,
                                                 block_id_t block_id,
                                                 FileBuffer& buffer) {
  // WriteTemporaryBuffer assumes that we never write a buffer below
  ...
  if (buffer.AllocSize() == GetBlockAllocSize()) {
    ...
    temporary_directory.handle->GetTempFile().WriteTemporaryBuffer(block_id,
                                                                   buffer);
    return;
  }
  ...
  auto handle = fs.OpenFile(
      path, FileFlags::FILE_FLAGS_WRITE | FileFlags::FILE_FLAGS_FILE_CREATE);
  temporary_directory.handle->GetTempFile().IncreaseSizeOnDisk(
      buffer.AllocSize() + sizeof(idx_t));
...
unique_ptr<FileBuffer> StandardBufferManager::ReadTemporaryBuffer(
    MemoryTag tag, BlockHandle& block, unique_ptr<FileBuffer> reusable_buffer) {
  ...
  if (temporary_directory.handle->GetTempFile().HasTemporaryBuffer(id)) {
    // This is a block that was offloaded to a regular .tmp file, the file
    // contains blocks of a fixed size
    return temporary_directory.handle->GetTempFile().ReadTemporaryBuffer(
        id, std::move(reusable_buffer));
  }

  // This block contains data of variable size so we need to open it and read it
  // to get its size.
  idx_t block_size;
  ...
  auto buffer = ReadTemporaryBufferInternal(
      *this, *handle, sizeof(idx_t), block_size, std::move(reusable_buffer));
  handle.reset();
...
void StandardBufferManager::DeleteTemporaryFile(BlockHandle& block) {
  ...
  // check if we should delete the file from the shared pool of files, or from
  ...
  if (fs.FileExists(path)) {
    evicted_data_per_tag[uint8_t(block.GetMemoryTag())] -=
...
vector<TemporaryFileInformation> StandardBufferManager::GetTemporaryFiles() {
  ...
  fs.ListFiles(temporary_directory.path, [&](const string& name, bool is_dir) {
    ...
    // Another process or thread can delete the file before we can get its file
    // size.
    auto handle =
        fs.OpenFile(name, FileFlags::FILE_FLAGS_READ |
...
const char* StandardBufferManager::InMemoryWarning() {
  ...
  return "\nDatabase is launched in in-memory mode and no temporary directory "
         "is specified."
...
// Buffer Allocator
//===--------------------------------------------------------------------===//
data_ptr_t StandardBufferManager::BufferAllocatorAllocate(
  ...
  auto reservation = data.manager.EvictBlocksOrThrow(
      MemoryTag::ALLOCATOR, size, nullptr,
...
data_ptr_t StandardBufferManager::BufferAllocatorRealloc(
  ...
  r.Resize(size);
  r.size = 0;
  return Allocator::Get(data.manager.db)
      .ReallocateData(pointer, old_size, size);
}
