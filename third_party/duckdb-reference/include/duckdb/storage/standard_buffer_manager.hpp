...
//! The BufferManager is in charge of handling memory management for a single
//! database. It cooperatively shares a BufferPool with other BufferManagers,
//! belonging to different databases. It hands out memory buffers that can be
//! used by the database internally, and offers configuration options specific
//! to a database, which need not be shared by the BufferPool, including whether
//! to support swapping temp buffers to disk, and where to swap them to.
class StandardBufferManager : public BufferManager {
 ...
 public:
  static unique_ptr<StandardBufferManager> CreateBufferManager(
      DatabaseInstance& db, string temp_directory);
  static unique_ptr<FileBuffer> ReadTemporaryBufferInternal(
  ...
  shared_ptr<BlockHandle> RegisterTransientMemory(const idx_t size,
                                                  const idx_t block_size) final;
  //! Registers an in-memory buffer that cannot be unloaded until it is
  //! destroyed. This buffer can be small (smaller than the block size of the
  //! temporary block manager). Unpin and Pin are NOPs on this block of memory.
  ...
  DatabaseInstance& GetDatabase() override { return db; }
 ...
 protected:
  ...
  //! destroyed instead of evicted, if true, it will be destroyed, if false, it
  //! will be written to a temporary file so it can be reloaded If we want to
  //! change this, e.g., to immediately destroy the buffer upon unpinning, we
  //! can call BlockHandle::SetDestroyBufferUpon The resulting buffer will
  //! already be allocated, but needs to be pinned in order to be used. This
  //! needs to be private to prevent creating blocks without ever pinning them:
  ...
  static data_ptr_t BufferAllocatorAllocate(PrivateAllocatorData* private_data,
                                            idx_t size);
  static void BufferAllocatorFree(PrivateAllocatorData* private_data,
                                  data_ptr_t pointer, idx_t size);
  static data_ptr_t BufferAllocatorRealloc(PrivateAllocatorData* private_data,
                                           data_ptr_t pointer, idx_t old_size,
                                           idx_t size);

  //! When the BlockHandle reaches 0 readers, this creates a new FileBuffer for
  //! this BlockHandle and overwrites the data within with garbage. Any readers
  //! that do not hold the pin will notice
  void VerifyZeroReaders(BlockLock& l, shared_ptr<BlockHandle>& handle);

  void BatchRead(vector<shared_ptr<BlockHandle>>& handles,
 ...
 protected:
  ...
  // so we need to store information related to the temporary directory before
  // it's created
  struct TemporaryFileData {
    //! The directory name where temporary files are stored
    string path;
    //! Lock for creating the temp handle (marked mutable so 'GetMaxSwap' can be
    //! const)
    mutable mutex lock;
 ...
 protected:
  ...
  //! Allocator associated with the buffer manager, that passes all allocations
  //! through this buffer manager
