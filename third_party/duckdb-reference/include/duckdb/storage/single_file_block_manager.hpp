...
//! SingleFileBlockManager is an implementation for a BlockManager which manages
...
class SingleFileBlockManager : public BlockManager {
 ...
 public:
  ...
  //! Loads an existing database. We pass the provided block allocation size as
  //! a parameter to detect inconsistencies with the file header.
  ...
  //! Write the header to disk, this is the final step of the checkpointing
  //! process
 ...
 private:
  ...
  void LoadFreeList();
  //! Initializes the database header. We pass the provided block allocation
  //! size as a parameter
  ...
  void VerifyBlocks(
      const unordered_map<block_id_t, idx_t>& block_usage_count) override;
 ...
 private:
  ...
  //! The list of multi-use blocks (i.e. blocks that have >1 reference in the
  //! file) When a multi-use block is marked as modified, the reference count is
  //! decreased by 1 instead of directly Appending the block to the
  //! modified_blocks list
  ...
  idx_t meta_block;
  //! The current maximum block id, this id will be given away first after the
  //! free_list runs out
