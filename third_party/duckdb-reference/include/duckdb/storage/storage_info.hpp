...
struct Storage {
  ...
  //! The minimum block allocation size. This is the minimum size we test in our
  //! nightly tests.
  constexpr static idx_t MIN_BLOCK_ALLOC_SIZE = 16384ULL;
  //! The maximum block allocation size. This is the maximum size currently
  //! supported by duckdb.
  ...
  //! Ensures that a user-provided block allocation size matches all
  //! requirements.
  static void VerifyBlockAllocSize(const idx_t block_alloc_size);
};

//! The version number default, lower and upper bounds of the database storage
//! format
...
struct DatabaseHeader {
  ...
  //! The allocation size of blocks in this database file. Defaults to
  //! default_block_alloc_size (DBConfig).
