...
//! databases. It defines memory limits and implements priority eviction among
...
class BufferPool {
 ...
 public:
  ...
  //! Set a new memory limit to the buffer pool, throws an exception if the new
  //! limit is too low and not enough blocks can be evicted
  void SetLimit(idx_t limit, const char* exception_postscript);

  //! If bulk deallocation larger than this occurs, flush outstanding
  //! allocations
 ...
 protected:
  ...
  //! If the "buffer" argument is specified AND the system can find a buffer to
  //! reuse for the given allocation size "buffer" will be made to point to the
  //! reusable memory. Note that this is not guaranteed. Returns a pair.
  //! result.first indicates if eviction was successful. result.second contains
  //! the reservation handle, which can be moved to the BlockHandle that will
  //! own the reservation.
 ...
 protected:
  ...
  struct MemoryUsage {
    //! The maximum difference between memory statistics and actual usage is 2MB
    //! (64 * 32k)
  ...
  //! Record timestamps of buffer manager unpin() events. Usable by custom
  //! eviction policies.
  bool track_eviction_timestamps;
  //! Eviction queues
  vector<unique_ptr<EvictionQueue>> queues;
  //! Memory manager for concurrently used temporary memory, e.g., for physical
  //! operators
  unique_ptr<TemporaryMemoryManager> temporary_memory_manager;
  //! To improve performance, MemoryUsage maintains counter caches based on
  //! current cpu or thread id, and only updates the global counter when the
  //! cache value exceeds a threshold. Therefore, the statistics may have slight
  //! differences from the actual memory usage.
