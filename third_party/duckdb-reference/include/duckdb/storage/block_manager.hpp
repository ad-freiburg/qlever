...
//! concrete types implement specific block storage strategies.
class BlockManager {
 public:
  BlockManager() = delete;
 ...
 public:
  ...
  //! Mark a block as "free"; free blocks are immediately added to the free list
  //! and can be immediately overwritten
  virtual void MarkBlockAsFree(block_id_t block_id) = 0;
  //! Mark a block as "used"; either the block is removed from the free list, or
  ...
  //! Whether or not the attached database is a remote file (e.g. attached over
  //! s3/https)
  ...
  //! Sets the block allocation size. This should only happen when initializing
  //! an existing database. When initializing an existing database, we construct
  //! the block manager before reading the file header, which contains the
  //! file's actual block allocation size.
 ...
 private:
  ...
  //! The allocation size of blocks managed by this block manager. Defaults to
  //! DEFAULT_BLOCK_ALLOC_SIZE for in-memory block managers. Default to
  //! default_block_alloc_size for file-backed block managers. This is NOT the
  //! actual memory available on a block (block_size).
