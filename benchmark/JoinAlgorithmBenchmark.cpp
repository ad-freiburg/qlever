...
static size_t createOverlapRandomly(IdTableAndJoinColumn* const smallerTable,
  ...
  */
  ...
  /*
  Go through the distinct element of the smaller table join column and mark,
  which will be overwritten.
  */
  ad_utility::HashMap<const ValueId, ValueId> smallerTableElementToNewValue{};
  std::ranges::for_each(
      ad_utility::HashSet<ValueId>(smallerTableJoinColumnRef.begin(),
                                   smallerTableJoinColumnRef.end()),
      [&randomDouble, &probabilityToCreateOverlap,
       &smallerTableElementToNewValue, &amountValueInBiggertTableJoinColumn,
       &randomBiggerTableElement](auto& id) {
      ...
      [&smallerTableElementToNewValue, &amountValueInBiggertTableJoinColumn,
       &newOverlapMatches](auto& id) {
        if (auto newValueIterator = smallerTableElementToNewValue.find(id);
          ...
          id = newValue;
          newOverlapMatches += amountValueInBiggertTableJoinColumn.at(newValue);
        }
      });
