...
@returns The minimum amount of new overlap matches created. That is, the amount
...
static size_t createOverlapRandomly(IdTableAndJoinColumn* const smallerTable,
  ...
  /*
  The minimum amount of new overlap matches, that are generated by this
  function. Because of how long it takes to count the amount of copies of a
  specific value in the bigger table, more than 10 times longer than without on
  tables of advanced size, we do not do it.
  */
  size_t minNewOverlapMatches{0};

  // Create the overlap.
  std::ranges::for_each(
      smallerTableJoinColumnRef,
      [&randomDouble, &probabilityToCreateOverlap, &biggerTableJoinColumnRef,
       &randomBiggerTableRow, &minNewOverlapMatches](auto& id) {
        // Only do anything, if the probability is right.
        if (randomDouble() <= probabilityToCreateOverlap) {
          // Create an overlap.
          id = biggerTableJoinColumnRef[randomBiggerTableRow()];
          minNewOverlapMatches += 1;
        }
      });

  return minNewOverlapMatches;
}
