...
static size_t createOverlapRandomly(IdTableAndJoinColumn* const smallerTable,
  ...
  std::vector<std::reference_wrapper<const ValueId>> biggerTableJoinColumnRef{
      ad_utility::transform(
          biggerTable.idTable.getColumn(biggerTable.joinColumn),
          [](const ValueId& id) { return std::cref(id); })};

  // Binary search can be used, if the vector is sorted..
  auto valueIdIdentity = [](const ValueId& id) -> decltype(auto) { return id; };
  std::ranges::sort(biggerTableJoinColumnRef, {}, valueIdIdentity);
  ...
  function.
  Because the calculation, how many copies of a value are in the join column of
  the bigger table, is expensive, we save already calculated amounts.
  */
  size_t newOverlapMatches{0};
  ad_utility::HashMap<ValueId, size_t> lookupTable;
  auto countNumEntriesBiggerTableJoinColumn = [&lookupTable, &valueIdIdentity,
                                               &biggerTableJoinColumnRef](
                                                  const ValueId& id) {
    if (auto findResultIterator = lookupTable.find(id);
        findResultIterator != lookupTable.end()) {
      return (*findResultIterator).second;
    } else {
      return (*lookupTable
                   .emplace(id,
                            std::ranges::equal_range(biggerTableJoinColumnRef,
                                                     id, {}, valueIdIdentity)
                                .size())
                   .first)
          .second;
  ...
  std::ranges::for_each(
      ...
      [&randomDouble, &probabilityToCreateOverlap, &biggerTableJoinColumnRef,
       ...
       &countNumEntriesBiggerTableJoinColumn](auto& id) {
        ...
        if (randomDouble() <= probabilityToCreateOverlap) {
          ...
          id = biggerTableId;
          newOverlapMatches +=
              countNumEntriesBiggerTableJoinColumn(biggerTableId);
        }
      });
