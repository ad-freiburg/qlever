...
std::string BenchmarkConfigurationOption::valueTypeToString(
...
// ____________________________________________________________________________
auto BenchmarkConfigurationOption::createValueTypeWithEmptyOptional(
    const size_t& typeIndex) -> ValueType {
  ValueType toReturn;
...
void BenchmarkConfigurationOption::setValueWithJson(
  ...
  */
  ...
  std::visit(
      [&json, this]<typename T>(const std::optional<T>&) {
...
// ____________________________________________________________________________
auto BenchmarkConfigurationOption::getActualValueType() const -> size_t {
  return value_.index();
}
