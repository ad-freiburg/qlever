...
// ____________________________________________________________________________
BenchmarkRegister::BenchmarkRegister(BenchmarkPointer&& benchmarkClassInstance){
...
-> std::vector<ResultEntry> {
  ...
  [](const auto& pointer)->ResultEntry{
      return (*pointer);
  });
}
...
auto BenchmarkResults::getGroups() const -> std::vector<ResultGroup> {
  ...
  [](const auto& pointer)->ResultGroup{
      return (*pointer);
  });
}
...
auto BenchmarkResults::getTables() const -> std::vector<ResultTable>{
  ...
  [](const auto& pointer)->ResultTable{
      return (*pointer);
  });
}
